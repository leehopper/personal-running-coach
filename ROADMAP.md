...
**Active slice:** **Slice 2b (Workout Logging proper) — in progress; 7 of 8 stacked PRs merged (#134 PR1 `plan-anchor` + #136 PR6a `fe-infra`, 2026-06-01; #145 PR2 `workoutlog-entity`, 2026-06-05; #147 PR3 `create` + #151 PR4 `query` + #154 PR5 `context-assembler`, 2026-06-06; #156 PR6b `log-form`, 2026-06-07); PR7 `history+e2e` next.** Slice 2 (Workout Logging) was decomposed 2026-05-18 into **2a Frontend Visual Foundation** (palette + semantic design tokens, light/dark, Tailwind v4 `@theme`, shadcn/ui install, typography/spacing scale, surface migration) and **2b Workout Logging proper** (data model, endpoints, `ContextAssembler` extension, log form / today's card / history list). **Sub-project 2a is complete — merged 2026-05-30** across PRs #111–#114 (foundation → contrast-gate → auth/onboarding migration → home/settings + dark-mode toggle), spec `docs/specs/15-spec-slice-2a-frontend-foundation/`. Slices 0, 1, and 1B are also complete and merged.
**Next step:** **Continue Slice 2b** per the signed-off 8-PR strategy — **PR1 `plan-anchor` ✅ merged (#134, 2026-06-01)** (the `PlanStartDate` calendar anchor + `PlanCalendar` server-side date→slot mapper, DEC-076 Unit 1) and **PR6a `fe-infra` ✅ merged (#136, 2026-06-01)** (date-derived `resolveCurrentWeek`, app-wide `<Toaster>` mount, `FormMessage` `role="alert"`), and **PR2 `workoutlog-entity` ✅ merged (#145, 2026-06-05)** (the `WorkoutLog` EF entity + EF-10 optional complex-type prescription snapshot + `WorkoutMetricKeys` + the repo's first `ValueConverter`s + migration + repository, DEC-072/DEC-076 Unit 2), and **PR3 `create` ✅ merged (#147, 2026-06-06)** (the `POST /api/v1/workouts/logs` create endpoint — server-authoritative snapshot resolution over the active plan + EF-native idempotency on a unique `(UserId, IdempotencyKey)` index, DEC-077 Unit 3), and **PR4 `query` ✅ merged (#151, 2026-06-06)** (the DB-driven `POST /api/v1/workouts/logs/query` keyset history endpoint — newest-first, opaque keyset cursor, server-clamped page size, with sort/page/limit all executing in a single SQL query; DEC-076 § C Unit 4), and **PR5 `context-assembler` ✅ merged (#154, 2026-06-06)** (the `ContextAssembler` extension injecting recent logged workouts' notes + metrics as compact Layer-1 one-liners — shared `WorkoutMetricKeys.Metadata` + `RecentLogFormatter`, never collapsed in the overflow cascade — with an eval proof, DEC-076 Unit 5), and **PR6b `log-form` ✅ merged (#156, 2026-06-07)** (the `/log` route logging form + today's-card "Log" action — RHF/Zod with the schema derived from the generated create-request Zod, pessimistic create + success toast, DEC-075/DEC-076 Unit 6). Remaining order: **PR7 history+E2E** (next: **PR7** — the frontend workout-history surface, an ISO-week-grouped sparse `<dl>` list over the `POST .../query` endpoint, plus the slice's Playwright E2E coverage, Unit 7). The 2b spec, 7 Gherkin `.feature` files, and the stacked-PR strategy are written, working-tree-only at `docs/specs/16-spec-slice-2b-workout-logging/` (gitignored). Pre-spec research (Batch 28 → DEC-072–075) and the product/UX brainstorm (2026-05-31) are complete — the four questions resolved as: (A) **snapshot-on-log** `WorkoutLog`↔prescription (server-authoritative nullable prescription snapshot, EF-10 complex type; off-plan = null; no minted id; server-side date→slot matching via a new `PlanStartDate` anchor; supersedes the stale cycle-plan `PlannedWorkoutId`; recorded as **DEC-076**), (B) dedicated **`/log` route** + success toast, (C) **DB-driven `POST .../query`** keyset history endpoint, ISO-week-grouped display, (D) **raw** history (sparse `<dl>` per DEC-075) + a `ContextAssembler` extension injecting compact layer-1 one-liners with selective RPE/HR absence markers. See `docs/plans/mvp-0-cycle/slice-2-logging.md` § Brainstorm resolutions and `docs/specs/16-spec-slice-2b-workout-logging/` (spec + `pr-strategy.md`).
**Blockers:** None.
