...
**Spec 11 — TestPaceCalculator migration + VDOT residue scrub (2026-04-18, complete on `refactor/11-testpacecalc-migration`):**
- Closes both DEC-042 follow-ups in one branch across three atomic commits (T01 → T02 → T03).
- **T01 (`6557c6d`):** Deleted `TestPaceCalculator.cs` (123 LOC lookup bridge). Migrated all four race-carrying profiles (Lee, Maria, James, Priya) in `TestProfiles.cs` to call the production `PaceZoneCalculator`. Rewrote `TestProfilesTests` pace-value assertions to use structural invariants (Fast<Slow, zone ordering E.Slow > E.Fast > M > T > I > R > F, `FastRepetitionPace` non-null for race-carrying profiles / null for Sarah) instead of hard-coded lookup values.
- **T02 (`e2d9862`):** Renamed `FitnessEstimate.EstimatedVdot` → `EstimatedPaceZoneIndex` with ripple through every consumer (ContextAssembler, PlanConstraintContext/Evaluator, TestProfiles, TestProfilesTests, PlanGenerationEvalTests). Scrubbed four `AssessmentBasis` VDOT literals to "pace-zone index" phrasing. Updated `RaceTime.cs:4` XML doc. Updated `context-injection-v1.yaml` token `{EstimatedVdot}` → `{EstimatedPaceZoneIndex}`. Renamed seven VDOT-bearing test method identifiers. Added a new parameterized `[Theory]` `AssembleAsync_FullAssembledPrompt_ContainsNoTrademarkedTermForAnyProfile` with 5 InlineData asserting no "VDOT" in the concatenated SystemPrompt + StartSections + MiddleSections + EndSections for any profile. Preserved the existing `SystemPrompt.Should().NotContain("VDOT")` guard (R02.9 semantics).
- **T03 (`4abb3ce`):** Full-script re-record via `backend/tests/scripts/rerecord-eval-cache.sh`. 22 eval-cache entries regenerated (17 Sonnet + 5 Haiku). All `entry.json` TTLs patched to `9999-12-31T23:59:59Z` per DEC-039. Record pass 6m 56s (≈24 live API calls), Replay pass 240ms with zero cache misses. Sarah's 2 entries kept identical cache-key hashes (her prompt is unchanged); the other 15 Sonnet + 5 Haiku got new hashes.
- **Verified residue:** `grep -rin "vdot" backend/src backend/tests` now returns only carve-out-exempt hits — live guard literals in `ContextAssemblerTests` (the term being scanned for), and the math comment in `DanielsGilbertEquationsTests.cs:73` (explicit R02.8 carve-out). Zero hits in source prompt YAML, model properties, or test profile data.
- Tests: 563 → 572 passing (+5 new Theory cases + 4 net from test rewrite/renames), 0 failures, 1 opt-in skip unchanged.
...
**DEC-042 follow-ups — CLOSED 2026-04-18 (spec 11 shipped on `refactor/11-testpacecalc-migration`):**
Both follow-ups were closed together in a single 3-commit branch — `TestPaceCalculator` deleted and all four race-carrying profiles migrated to `PaceZoneCalculator`; `FitnessEstimate.EstimatedVdot` renamed to `EstimatedPaceZoneIndex` and rippled through every consumer; `RaceTime` XML doc and four `AssessmentBasis` literals scrubbed; VDOT-bearing test method names renamed; a new parameterized `ContextAssemblerTests` Theory guards the full assembled prompt against future "VDOT" regressions for all 5 profiles; Sonnet + Haiku eval cache re-recorded via `rerecord-eval-cache.sh` with 9999-12-31 TTL applied to all 22 entries. Replay-mode CI passes 572/573 with only the opt-in SmokeTests skip.
