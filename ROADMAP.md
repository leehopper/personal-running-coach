...
- Two internal C# identifiers still reference the VDOT term: `FitnessEstimate.EstimatedVdot` property and an XML doc comment on `RaceTime`. Out of user-facing scope and harmless, but worth renaming to `PaceZoneIndex` in the next touch on either file for consistency with the DEC-043 rename.
