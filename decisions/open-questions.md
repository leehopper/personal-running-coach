...
| Question | Status | Notes |
|----------|--------|-------|
| How does the AI communicate plan downgrades without demoralizing users? | Decided | R-010 produced a concrete pattern: Reassure → Frame as phase → Show trajectory → Give agency. Lead with fitness retention science, always show the path back up, frame rest as investment not punishment. Anti-patterns: leading with the reduction, being clinical, minimizing. See coaching-persona.md scenario playbook and DEC-027. |
| How does the AI handle users who consistently over-perform on easy days? | Decided | R-010 confirmed this is a confidence problem, not a knowledge problem. Pattern: Acknowledge → Data → Rationale → Elicit. Key reframes: "You cannot run too slowly on a recovery day, only too fast," Kipchoge's easy pace as reference. Anti-patterns: accusatory, directive, fear-based language. See coaching-persona.md and DEC-027. |
| What does the "coach training phase" look like from the user's perspective? | Decided | R-010 produced a concrete pattern: Set expectations → Deliver quick value → Phase data collection → Name the learning period. Lead with value not data collection, explicitly acknowledge early plans are generic, promise personalization improves by week three, frame early runs as a conversation that teaches the AI. See coaching-persona.md and DEC-027. |
| How does the AI handle injury-specific situations? | Decided | R-010 + R-011 provide the complete answer. Conversational approach: "concerned coach" framing (R-010). Safety boundaries: universal five-stage return-to-run framework with traffic-light pain monitoring, injury-specific coaching modifications for 7 common injuries, femoral neck stress fracture as emergency referral. Pattern: Affirm disclosure → State limits → Recommend action → Offer what's in scope → Maintain relationship. Iron rule: "If in doubt, refer out." See coaching-persona.md, safety-and-legal.md, DEC-028, DEC-030. |
