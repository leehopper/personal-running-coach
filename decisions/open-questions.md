...
| How does the system handle AI output failures? | Leaning | R-001 clarifies this significantly: the LLM should NOT generate raw plans/paces — those come from deterministic computation (VDOT tables, load math, safety checks). The LLM handles explanation, adaptation reasoning, and coaching conversation. This architecture inherently prevents most dangerous hallucinations because the computation layer gates what the LLM can prescribe. Remaining risk: LLM could give bad qualitative advice (e.g., "push through the pain"). Need mandatory uncertainty declarations for injury, medical conditions, and edge cases. |
