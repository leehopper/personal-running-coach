...
> **Trademark rule.** User-facing surface (coaching prompt YAML under `src/RunCoach.Api/Prompts/`, API responses, generated plan narrative, error messages) must use "Daniels-Gilbert zones" or "pace-zone index" — never the trademarked term `VDOT`. The mark is enforced by The Run SMART Project LLC (Runalyze precedent). Calculator classes are `PaceZoneIndexCalculator`, `PaceZoneCalculator`, and `HeartRateZoneCalculator` per DEC-042; the `FitnessEstimate.EstimatedPaceZoneIndex` property, `RaceTime` XML doc, and `TestProfiles` `AssessmentBasis` strings were scrubbed in spec 11 (2026-04-18). `ContextAssemblerTests` has a parameterized Theory asserting the trademarked term (case-insensitive) does not appear in the full assembled prompt for any of the 5 test profiles. Slice 3B F2 added the LLM-output side: `TrademarkScrubber` (`Modules/Coaching/`) deterministically rewrites the term to "pace-zone index" on every `ICoachingLlm` response before deserialization (warn-logged), the active generation prompts carry an explicit vocabulary rule, and `TrademarkProseGuard` asserts every prose field of the cached eval outputs stays clean. The only in-repo occurrences of the literal term are carve-out-exempt: policy/rule docs (root `CLAUDE.md`, `REVIEW.md`, `README.md`, `NOTICE`), trademark-guard and scrub-coverage test files (which must spell the term to prove it is caught — e.g. `ContextAssemblerTests`, `OnboardingPromptTests`, `TrademarkScrubberTests`, the eval trademark guards), and `DanielsGilbertEquationsTests.cs` (math comment). See root `CLAUDE.md` and `NOTICE` for full context.
...
- `global.json (repo root)` selects the MTP runner via `test.runner: Microsoft.Testing.Platform` — the only knob .NET 10+ SDK honors. The legacy `<TestingPlatformDotnetTestSupport>` MSBuild property is silently ignored on .NET 10 SDK and routes `dotnet test` through VSTest's MSBuild bridge, which deadlocks on macOS waiting on the orphaned MTP test exe pipe (`Monitor_Wait` / `Microsoft.Testing.Platform.MSBuild`). The MTP-family transitives (`Microsoft.Testing.Platform.MSBuild`, `Microsoft.Testing.Extensions.Telemetry`, `Microsoft.Testing.Extensions.TrxReport`, `coverlet.MTP`) are pinned to the same 2.x line via `Directory.Packages.props` + `CentralPackageTransitivePinningEnabled`, and `xunit.v3` is replaced by the explicit `xunit.v3.core.mtp-v2` + `xunit.v3.assert` package pair so the xunit MTP adapter resolves the same `IDataConsumer` / `IOutputDevice.DisplayAsync` API surface as the rest of the stack. Lift the pins when `xunit.v3` ships an MTP-2.x default release.
- Local invocation: `dotnet test --solution RunCoach.slnx --no-build` (positional path is rejected by MTP-native; use `--solution` or `--project`).
- Coverage uses `coverlet.MTP` (MTP-native) instead of `coverlet.msbuild` (VSTest-bridge-only). CI passes `--coverlet --coverlet-output-format opencover --coverlet-output-format cobertura --results-directory backend/TestResults`; SonarQube ingests via the `*.opencover.xml` glob, Codecov via the cobertura sibling.
