...
/// and invalidate its <c>WorkoutLog</c> RTK-query tag. On a replayed idempotency
/// key the prior id is re-read straight from the <c>WorkoutLog</c> row via its
/// unique <c>(UserId, IdempotencyKey)</c> index (DEC-077) — there is no separate
/// idempotency marker on this path.
/// </summary>
