...
public sealed partial class PlanGenerationService : IPlanGenerationService
{
    ...
    /// <summary>
    /// Marker label that begins the per-retry micro correction suffix appended to the micro user
    /// message when a bounded meso/micro consistency-rejection retry fires (DEC-088). Distinct name
    /// from <see cref="MacroCorrectionLabel"/> (same wire bytes) so the micro retry tests can locate
    /// it symmetrically and confirm the attempt-0 message carries no suffix.
    /// </summary>
    ...
    /// <summary>
    /// Upper bound on the configured <see cref="_settings"/> micro-validation retry count, so a
    /// misconfigured large value can't burn dozens of extra LLM calls per plan-generation chain.
    /// Mirrors <see cref="MaxAllowedMacroValidationRetries"/> (DEC-088 / F-LIVE-2).
    /// </summary>
    private const int MaxAllowedMicroValidationRetries = 5;
        ...
        CancellationToken ct)
    {
        ...
        // Macro + micro attempt counters are hoisted above the try so the failure catch can stamp
        // them on the completion metric alongside the success path (DEC-087 / DEC-088 D6).
        var macroAttempts = 0;
        var microAttempts = 0;
        ...
        try
        {
            ...
            // Tier 3 — micro week-1 detail, with a bounded corrective-hint retry on meso/micro
            // consistency rejection (DEC-088 / F-LIVE-2, mirroring the macro tier's DEC-087 retry).
            // The two layers are generated by independent structured-output calls and can silently
            // disagree on the week-1 run schedule (differing run-day count or a swapped workout
            // type), which the home page renders side by side. The meso week is the source of truth:
            // it has already been generated + paid for, so a re-roll re-invokes only the micro tier
            // (the last call, no meso/macro work wasted) with a correction suffix naming the exact
            // run-day schedule to reproduce. The suffix rides the never-cached user message, so the
            // `Ephemeral1h` system-block cache is untouched — attempt 0 is byte-identical to the
            // no-retry path. After `MicroValidationMaxRetries` extra attempts a rejection is
            // terminal: throw before event staging, so the caller's Marten transaction aborts with
            // nothing committed (onboarding maps it to the same 422 as a macro rejection).
            ...
            {
                microAttempts++;
                var (candidate, microUsage, candidateChars) = await InvokeTierAsync<MicroWorkoutListOutput>(
                    tier: TierMicro,
                    planId: planId,
                    systemPrompt: systemPrompt,
                    userMessage: BuildMicroUserMessage(basePrompt, macro, weekOneMeso, microCorrection),
                    extraTags: null,
                    ct).ConfigureAwait(false);
                ...
                var microConsistency = MesoMicroConsistencyValidator.Validate(weekOneMeso, candidate);
                if (microConsistency.IsValid)
                {
                    micro = candidate;
                    microOutputChars = candidateChars;
                    break;
                }
            ...
            // macro validation retry (DEC-087) or a micro consistency retry (DEC-088) adds extra macro
            // or micro calls. Stamped as `total_calls` so a cost/volume dashboard reflects the
            // recovered-on-retry population the retries exist to make visible (`MacroAttempts` /
            // `MicroAttempts` carry the per-tier retry deltas separately).
            var totalLlmCalls = macroAttempts + MesoWeekCount + microAttempts;
            ...
            chainActivity?.SetTag(PlanGenerationTagNames.MicroAttempts, microAttempts);
            ...
            var tags = new TagList
            {
                ...
                { PlanGenerationTagNames.MicroAttempts, microAttempts },
        ...
        catch (Exception ex)
        {
            ...
            // Stamp the macro + micro attempt counts on the span for the failure path too (DEC-087 /
            // DEC-088 D6): a trace-only view of a macro- or micro-exhaustion rejection must show how
            // many attempts were made at the failing tier.
            chainActivity?.SetTag(PlanGenerationTagNames.MacroAttempts, macroAttempts);
            chainActivity?.SetTag(PlanGenerationTagNames.MicroAttempts, microAttempts);
            stopwatch.Stop();
    ...
    /// progression context it needs to lay out detailed workouts. When
    /// <paramref name="correction"/> is non-null (a bounded meso/micro consistency-rejection retry,
    /// DEC-088) it is appended after a blank-line separator so the model sees the exact run-day
    /// schedule to reproduce. A null correction yields bytes identical to the pre-DEC-088 micro
    /// message, preserving the attempt-0 input-prompt-stability contract.
    /// </summary>
    ...
    /// <summary>
    /// Appends a compact "day: type" summary of the micro workouts (run days only; cross-train
    /// entries are excluded, matching the consistency validator's run-day scope) for the corrective
    /// hint. Renders <c>none</c> when the micro emitted no run workouts.
    /// </summary>
    private static void AppendMicroDaySummary(StringBuilder sb, MicroWorkoutListOutput micro)
    {
        var first = true;
        foreach (var workout in micro.Workouts)
        {
            if (workout.WorkoutType == WorkoutType.CrossTrain)
            {
                continue;
            }
            ...
            sb.Append(CultureInfo.InvariantCulture, $"{(DayOfWeek)workout.DayOfWeek}: {workout.WorkoutType}");
            first = false;
        }
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "Week-1 micro workouts rejected by meso/micro consistency (retries exhausted): PlanId={PlanId} Violation={Violation} Attempts={Attempts}")]
    private static partial void LogMicroConsistencyRejected(
        ILogger logger,
        Guid planId,
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "Week-1 micro workouts inconsistent with meso, retrying with correction: PlanId={PlanId} Violation={Violation} Attempt={Attempt}")]
    private static partial void LogMicroConsistencyRetry(
        ILogger logger,
        Guid planId,
        MesoMicroConsistencyViolation violation,
        int attempt);
