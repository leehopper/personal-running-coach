...
- **Active Slice:** Slice 3 (Coach), in progress — PR-A (backend) shipped 2026-07-15 as PR #288; PR-B (frontend turn kinds/timestamps/dividers/composer) shipped 2026-07-15 as PR #290; PR-C (adaptation + safety turns + danger-surface token) shipped 2026-07-15 as PR #292. PR-D (durable receipt + log-draft card states) is the last PR of the slice. Slice 2 (Today) shipped 2026-07-14.
- **Slice ledger:**
  | # | Slice | Completed | PR |
  |---|---|---|---|
...
- **Slice 3 PR progress:** **PR-A shipped** (2026-07-15, PR #288) — structured `LoggedRunSummary` persisted on the confirm-ack turn (DEC-091) + codegen; deep-review + full CodeRabbit addressment landed before merge (two follow-up comments after the first push, both addressed). **PR-B shipped** (2026-07-15, PR #290) — `UserTurn`/`CoachTextTurn`/`DateDivider` turn-kind dispatch, local-day grouping, shared live-exchange timestamp, composer restyle (48px clay send square, no-markdown body); 888/888 vitest at merge. Post-push review (1 CodeRabbit comment-hygiene finding + 8 deep-review findings, the latter posted as inline review comments) addressed before merge: a real bug fix (stale `liveTime` on an identical-text RETRY — now keyed off `isStreaming`'s transition, not message-text equality), `rows={1}` composer fix, a dead-code Fragment-key simplification, and comment-hygiene trims (dropped PR-sequencing/slice-forward-reference narrative per `REVIEW.md` § Code comments). **PR-C shipped** (2026-07-15, PR #292) — the adaptation turn rebuilt as the `PLAN ADJUSTED` card with calendar-date-anchored diff loci (UTC plan-anchor math, week-index fallback), `SafetyTurn` refactored to a shared `{ tier, content, role?, testId? }` core across the persisted + live paths (content always in full, AX-01, local `SafetyNotice` deleted), and the deferred `--danger-surface`/`--danger-text` tokens landed (the live retry affordance moved onto `bg-danger-surface`, resolving PR-B's interim `bg-secondary`). 912/912 vitest at merge; check-contrast 44 pairs. Post-push review (3 CodeRabbit + 3 self-review comments) addressed before merge: sentence-case safety headings (uppercase via CSS `.t-section-label`); a **real AA fix** moving the amber heading off the ungated `--warning` border-accent (~1.8:1 on the light card) onto a new gated `--warning-text` pair mirroring `--danger-text`; semantic `<h3>`/`t-body` markup; a stale `--danger-surface` CSS comment; deletion of four production-dead legacy diff helpers (behavioural tests retargeted to the live `*Parts` helpers); and an out-of-range `dayOfWeek` bounds-check so a malformed day degrades to the week index rather than a plausible-but-wrong calendar date. **Next: PR-D** — log-draft card saving/success/failure states + the durable `loggedRun` receipt PR-A now persists; only D depends on A.
- **Next Step:** **Build Slice 3 PR-D** in a fresh session from `docs/specs/slice-3-coach/spec.md` § 3 (read this plan + `slice-3-coach.md` first) — the log-draft card's saving / success-receipt / failure states plus the durable `loggedRun` receipt chip that renders the structured `LoggedRunSummary` PR-A persists (DEC-091). PR-D is the last PR of Slice 3 and the only one that depends on PR-A. Slices 3/4 are independent after Slice 1; numbered order is the default.
- **Blockers:** None.
