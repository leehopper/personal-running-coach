...
- **Active Slice:** Slice 7 (States, Light Pass & Cycle Close) — next to build; no PR yet. Slice 6 (Settings & Auth) shipped complete 2026-09-06 (PRs #365 / #364, DEC-089 D8). Slice 5 (Onboarding) shipped complete 2026-09-06 (PRs #361 / #363, DEC-089 D7). Slice 4 (Log & Log Book) shipped complete 2026-07-19 (PRs #302 / #304 / #306 / #308, DEC-089 D5).
- **Slice ledger:**
  | # | Slice | Completed | PR |
  |---|---|---|---|
  ...
  | 6 | Settings & Auth | 2026-09-06 | #365 / #364 |
- **Active Slice Spec:** Slice 7 (States, Light Pass & Cycle Close) has **no spec yet** — write it first from `docs/plans/split-ui-cycle/slice-7-states-close.md` (specs are gitignored working-tree artifacts, planned per-slice at build time). Slice 6's narrative is captured in the Slice 6 completion section below and the `ROADMAP.md` Cycle History row; its committed evidence (recon, two-family red-team, per-PR lens reports, fix lists, orchestrator runs and e2e mutation replays) is `docs/plans/split-ui-cycle/slice-6-evidence/` (PR-B under `pr-b/`).
- **Next Step:** **Begin Slice 7 (States, Light Pass & Cycle Close)** in a fresh session — write the slice spec from `docs/plans/split-ui-cycle/slice-7-states-close.md` (D1 states coverage audit; D2 interaction audit; D3 full light-mode pass; D4 a11y re-verification; D5 suite consolidation, which now owns the two journeys known red on `main` since Slice 5, the shared e2e account-setup helper, and the regenerate dialog's migration to the shared Dialog primitive if its focus-trap audit wants it; D6 the closing funded-key live pass that is also the MVP-0 F-LIVE-1/F-LIVE-2 done-gate), then run it on the DEC-092 recipe (`.claude/codex/README.md`). Read this plan's Status, the 2026-09-06 Captured rows, and `slice-7-states-close.md` first.
- **Blockers:** None.
...
| 2026-07-15 | **Slice 4 (Log & Log Book) spec recon found D4's "prescribed-title server-resolved against the stored snapshot" is half-broken** — `IsOnPlan = (Prescription != null)` is a clean snapshot read, but `WorkoutPrescriptionSnapshot` has **no title** (only `WorkoutType` + distance/duration/pace). The descriptive title lives on the LLM-authored `WorkoutOutput.Title`, which `RestructureDiffCalculator` diffs on adaptation → it **drifts** if re-resolved live, retroactively rewriting an old ledger row. | **Decided (spec §1): option A** — the D4 field carries the **frozen `WorkoutType` enum name** (as a nullable string, `PrescribedWorkoutType`), rendered to a title via the existing `WORKOUT_TYPE_LABELS`. Point-in-time correct, **zero migration/no new persistence**, and symmetric with the D1 banner (`CandidatePrescriptionDto` also carries only `WorkoutType`). Rejected option B (live re-resolution — drifts) and C (new persisted column — migration for a fidelity the design doesn't need). This is a **source clarification within the already-blessed DEC-089 D5(b)**, not a new wire delta beyond the cycle's four (contrast Slice 3's DEC-091, which *added* a field). Recommendation: a one-line note appended to DEC-089 in the decision log (builder ratifies at PR-A per spec § Builder review checkpoints); a full new DEC is not warranted. No LLM/prompt change → no eval re-record. |
| 2026-09-05 | **Slice 5 spec: DEC-089 D7's "DEC-074 manifest regen" does not apply as written.** The narrative is injected by C# composition (`ContextAssembler.BuildPlanGenerationUserMessage`), not a `Prompts/*.yaml` edit; `check-prompt-hashes.sh` hashes only the YAML, so the manifest is unchanged and the D7 regen step is a verified no-op. An empty narrative adds no prompt bytes, so existing fixtures stay byte-identical; the eval consequence narrows to recording ONE new fixture (`plan.dated-event-narrative.macro`). | Recorded as a build note under DEC-089 D7 and in the Slice 5 spec section 9. The sol red-team lens also read `backend/REVIEW.md`'s never-modify-an-event rule as forbidding the additive `AnswerCaptured.Narrative` field; ruled against on DEC-089 D7 plus the DEC-091 / Slice 2 additive-nullable precedent, and the rule text amended in PR-A to record that exception (C4). |
...
| 2026-09-06 | **Sign-out leaves `state.next` = `/settings`** on the login redirect (the guard records the protected location), so a re-login after signing out from Settings returns to Settings. | Harmless; noted in the Slice 6 spec. No change. |
| 2026-09-06 | **The regenerate dialog stays a custom modal** (own `role="dialog"`, `aria-modal`, testids) rather than the shared Dialog primitive; Slice 6 restyled it and added the building phase without migrating it, to keep its preserved ARIA and testids. | Migrate to the shared primitive in Slice 7's states pass if the focus-trap audit wants it; otherwise post-cycle cleanup. |
| 2026-09-06 | **Regenerate failure copy stays the generic retry alert** (the design has none); the server's 400 intent-length rejection maps to the same alert. | Slice 7's states pass (D1) owns dedicated failure copy. |
| 2026-09-06 | **The password visibility toggle's accessible name (`Show password`) makes substring label queries ambiguous**: Testing Library and Playwright both match `aria-label` by substring, so every password lookup in the page specs and all seven Playwright journeys became `exact: true` in PR #364. Future specs must query the field exactly. | Done in Slice 6 PR-B; recorded here so the Slice 7 selector inventory (D5) keeps the exact form. |
| 2026-09-06 | **E2E account setup is duplicated per spec file** (each journey registers with its own stubs and helpers; the new `sign-out.spec.ts` copies the pattern). | Slice 7's suite consolidation (D5) owns a shared helper. |
| 2026-09-06 | **The generated-date test fixture is timezone-bound at the edges**: the Settings current-plan line formats `generatedAt` in the runner's local time; the fixture is pinned to mid-day UTC so every zone from UTC-11 to UTC+11 renders `Jun 29, 2026`, but a UTC+12 to UTC+14 runner would render Jun 30. CI runs UTC. | Recorded residual; revisit only if a runner in those zones appears. |
| 2026-09-06 | **`VITE_APP_VERSION` was never set before Slice 6**, so error telemetry reported `unknown` and the OpenTelemetry service version `0.0.0-dev`. PR #365 defines it at build time from the package version (0.9.0) with an env override; both now report the real version. | Done; a future release bumps `package.json` and the Settings footer literal test together. |
...
### Slice 6 — Settings & Auth ✅ Complete (2026-09-06, PRs #365/#364)

**Requirements:** [`./slice-6-settings-auth.md`](./slice-6-settings-auth.md) (design doc — the build source of truth was `docs/specs/slice-6-settings-auth/spec.md`, gitignored; committed evidence under `./slice-6-evidence/`)

**Acceptance — "I can…"**

- [x] …see Settings as rule sections: THE PLAN (current-plan line from existing plan data + restyled regenerate flow with the BUILDING surface), APPEARANCE and UNITS segmented controls, ACCOUNT with my email and a working SIGN OUT, and the mono version footer.
- [x] …sign in on the poster auth screen (58px wordmark, mono tagline, eye-toggle password) and register on its START HERE mirror.
- [x] …sign out and land on login with all per-user caches cleared, in every open tab.
...
**Shipped:** Settings recomposed onto the Alpine rule sections and auth rebuilt as the SPLIT poster, frontend only, as two independent PRs built in parallel from `main` (disjoint file lists except one byte-identical e2e line). **PR-A** (#365) — THE PLAN current-plan line from the loaded plan (segments omitted when missing, invalid dates passed through), REGENERATE PLAN as a clay outline with `dark:` overrides and the mono warning, the regenerate dialog restyled per sheet 5h with its body kept mounted so the panel swaps for the `BuildingPlanSurface` overlay during the synchronous POST (success navigates to Today, failure returns the panel with the same intent and key), APPEARANCE and UNITS as `SegmentedControl`s on the unchanged theme and units wiring, ACCOUNT with the signed-in email and the first SIGN OUT (`useSignOut`: logout POST, then `loggedOut`, `resetApiState`, broadcast in `finally`; the receiver purges too; the pulled-forward cache-reset follow-up is closed), the mono version footer from `VITE_APP_VERSION` defined from the package version (0.9.0), two muted-text contrast pairs (27 / 54). **PR-B** (#364) — `AuthPosterHeader`, 48px mono-labelled fields, `PasswordVisibilityToggle` beside the `FormControl`-wrapped input, hidden login h1 and visible `START HERE`, 52px primary with an empty 52px OAuth reserve, secondary links with a 44px target and the canonical ring, the mono password-rules helper, and exact password lookups across the page specs and all seven Playwright journeys. **Review trail:** six recon lenses (five luna, one terra over the design source) and the orchestrator's rulings; a two-family spec red-team (opus REJECT with four blockers, all fixed before the builds: a measurably failing contrast pair, ambiguous password queries in unit and e2e specs, a loading-phase contradiction, a circular footer test; sol REJECT, three code-facing findings taken into round 1: the outline variant's dark fill under clay text at 3.84:1, link hit targets, form rhythm); per-PR mutation and conformance lenses (PR-A: 30 red, 7 green order gaps, the missing `Current plan` eyebrow; PR-B: 22 red, 3 untested geometry contracts); one fix round each; eleven e2e mutation replays from the session, all red; verify lenses. 

**Key risks (retired).** Sign-out was net-new user-reachable behavior, not a restyle — it got its own tests: five hook cases (POST-first ordering, purge proven by an empty query cache and a refetch, exact broadcast, second-store receiver purge, rejected-POST cleanup), the live guard flip, and two Playwright journeys including the second tab.
