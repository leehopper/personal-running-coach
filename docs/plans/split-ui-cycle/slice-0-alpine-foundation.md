...
> **Design doc — requirements, not a spec.** Parent: [`./cycle-plan.md`](./cycle-plan.md). Design source: `docs/design/split-alpine/HANDOFF.md` §§ 1–3, 6–8 + design sheets 4a/4b/5g. **Research gate: R-086 — CLEARED** (integrated 2026-07-07 with errata; artifact `docs/research/artifacts/batch-32a-self-hosted-fonts-vite-react.md`). The font architecture is locked in D4; spec-writing is unblocked.
...
- **D4 — Fonts self-hosted via static per-weight `@fontsource/*` packages (R-086, integrated 2026-07-07 with errata).** Pins verified live on npm: `@fontsource/barlow@5.2.8` (400/500/600), `@fontsource/barlow-condensed@5.2.8` (500/600/700/800), `@fontsource/ibm-plex-mono@5.2.7` (400/500/600) — 10 weight files, **latin subset only** via granular per-weight CSS imports. (The artifact's primary `@fontsource-variable/*` recommendation was refuted at integration: none of the three families ships a Fontsource variable package — npm 404 ×3, Fontsource API `variable: false`; its own designated static fallback is adopted.) Loading: `font-display: swap` + **`fontaine@0.8.0`** Vite-transform metric-matched fallbacks (`size-adjust`/`ascent-override`; iOS Safari 17+ honors them, older degrades gracefully — verify the condensed fallback visually, it needs a large `size-adjust`), with the generated `"… fallback"` family names appended to the `--font-condensed`/`--font-body`/`--font-mono` `@theme` tokens. Preload **exactly three** above-the-fold weights (Barlow 400, Barlow Condensed 700, Plex Mono 400) via Vite `?url` hashed imports with mandatory `crossorigin`, injected after the no-flash theme script; font files never go in `/public` (defeats hashing). Licensing: OFL-1.1 satisfied by the packaged license files + a repo `THIRD-PARTY-LICENSES.md` entry per family. Caveat: `tabular-nums` may be silently absent from the Google-Fonts-derived Barlow files — moot for this design (condensed numerals use `white-space: nowrap`; data columns are inherently-tabular Plex Mono); do not build tabular-Barlow dependence without a Wakamai Fondue check. No Google Fonts CDN at runtime.
...
2. **PR-B** — fonts (per D4: @fontsource static packages + fontaine + preload wiring + `THIRD-PARTY-LICENSES.md`) + typography role system.
...
- `docs/research/artifacts/batch-32a-self-hosted-fonts-vite-react.md` (R-086, integrated with errata — read the errata block first).
