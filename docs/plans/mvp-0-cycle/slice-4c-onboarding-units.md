...
- **Research gate**: per `CLAUDE.md` § Research Protocol and the pre-flag on the 2026-06-13 row, the form-first redesign lands a research artifact (R-085, `batch-31a`) before the 4C-onboarding spec. The units foundation is not gated.
...
- **Coverage is end-to-end across every numeric surface**: plan cards / today / upcoming, logged-workout **history**, the log confirmation card, and all pace labels — not just the plan view.
...
- **4C-units**: the `UserSettings` store + `GET`/`PUT` endpoints; the shared frontend unit-format module (distance + net-new pace formatter with the display spec); wiring the module across plan/today/upcoming, workout-log history, the log confirmation card, and pace labels; miles→km input conversion; the Settings-page Units toggle; DEC-086 (amend DEC-041 phasing); deletion of the dead `UserPreferences` string / `FormatPreferences` / `AssembleAsync` path.
...
- **R-085 (`batch-31a`) — form-first hybrid onboarding intake over an event-sourced Pattern-B flow.** Gates the 4C-onboarding spec only. Prompt at `docs/research/prompts/batch-31a-form-first-onboarding-redesign.md`; queued in `docs/research/research-queue.md`. Resume 4C-onboarding once the artifact lands at `docs/research/artifacts/batch-31a-*.md` and is integrated here. **4C-units proceeds in parallel.**

## Open / deferred items

- The DEC-084 open item (scope the onboarding restraint rubric to register-only criteria so short clarifier turns stop scoring 0.0) is a test-only change that 4C-onboarding may fold in if the redesign keeps any LLM-scored intake turn.
- Whether the form-first redesign eliminates the multi-slot free-text-merge eval scenarios the 2026-06-11 row asked for (a form collecting structured slots removes the failure mode being tested) is a 4C-onboarding spec-time call, informed by R-085.
