...
**Remaining close-out:** two funded-key eval re-records the Replay suite skips until present — F3's dated-event macro fixture and F4's `RestructurePlan` schema-`[Description]` curated re-record (DEC-083). The live browser done-gate itself is met.
