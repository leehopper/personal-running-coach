...
- **Active Slice:** Slice 1 (Onboarding → Plan) — implementation-ready. Slice 0 (Foundation) closed 2026-04-23 with PR #63 (Unit 3 Frontend Auth UX) on top of PR #49 (Unit 1 persistence substrate) and PR #50 (Unit 2 Auth API). End-to-end register → login → protected home → logout runs against a real browser under `__Host-` cookies + antiforgery. DEC-048 through DEC-056 captured the substrate decisions made along the way.
- **Active Slice Requirements:** `./slice-1-onboarding.md` (already amended with R-048 / DEC-047 lock-ins).
- **Active Slice Spec:** `docs/specs/13-spec-slice-1-onboarding/13-spec-slice-1-onboarding.md` written 2026-04-25, with five `.feature` files (54 BDD scenarios) alongside it. **Six demoable units** including new Unit 6 (`IPromptSanitizer` infrastructure). Updated 2026-04-25 to integrate R-066/R-067/R-068/R-069 findings — major restructure: plan generation is a plain DI service (NOT a Wolverine command), invoked inline within Unit 1's `[AggregateHandler]`; Pattern B nullable-slots schema for `OnboardingTurnOutput`; layered sanitizer wired in `ContextAssembler`; **handler bodies emit events, projections own EF state** (DEC-060) — `PlanLinkedToUser` event + `EfCoreSingleStreamProjection<UserProfile, Guid, RunCoachDbContext>` apply method replaces direct EF writes; `IIdempotencyStore` is Marten-document-backed, NOT EF-table-backed.
- **Task board:** 31 tasks (#84-#88 parents + #89-#113 sub-tasks + #114-#115 sanitizer) with dependency edges wired. T01.1 / T01.2 / T01.3 / T06.1 are immediately unblocked. #89 / #90 / #91 / #92 / #94 / #110 / #111 reshaped per DEC-060: drop `IdempotencyRecord` EF table (Marten doc instead); add 8th event (`PlanLinkedToUser`); projection signature updated to 3-type-param API; handler signatures drop `RunCoachDbContext` injection.
- **Next Step:** Begin Slice 1 implementation — all pre-implementation research resolved (R-066/R-067/R-068/R-069 all integrated as DEC-057/058/059/060). Four unblocked atomic tasks ready: #89 (UserProfile entity + migration), #90 (Marten-doc idempotency), #91 (events + Pattern B records + prompt YAML — now 8 events including `PlanLinkedToUser`), #115 (sanitizer + 25-case corpus). Recommended dispatch: parallel via `/cw-dispatch` — these four are independent.
- **Blockers:** None. All pre-implementation research resolved.
...
| 2026-04-21 | Slice 0 (R-058 / DEC-052 follow-ups) | Frontend (T03.x) Zod schemas must mirror `RegisterRequest` DataAnnotations; contract-test in `shared-contracts`. | Deferred as a T03 follow-up — Unit 3 shipped the schemas; contract-test still open. |
...
| 2026-04-27 | Slice 1 (PR #67 review carry-forward) | CodeRabbit (#141, inline 3146847034) flagged the second `GenerateStructuredAsync<T>` overload on `ICoachingLlm` (the one accepting `IReadOnlyDictionary<string, JsonElement>? schema` + `CacheControl?` markers) as exposing Anthropic-specific implementation details through what should be a vendor-neutral interface, and proposed wrapping schema + cache-control behind a `StructuredOutputOptions` abstraction. | **Deferred to post-MVP-0 tiered-routing workstream (DEC-038); revisit when adding a non-Anthropic `ICoachingLlm` implementation.** Pushed back: (a) `ClaudeCoachingLlm` is currently the **only** implementer of `ICoachingLlm` (verified via `rg ': ICoachingLlm'`), so the "vendor-neutral" argument has no second consumer to bind against. (b) DEC-038 explicitly defers the multi-vendor / `ModelTier` routing surface to post-MVP-0 — adding the abstraction now is YAGNI. (c) The schema-dictionary shape is portable across modern LLM APIs (OpenAI `response_format`, Anthropic `output_config.format`, Gemini `responseSchema` all accept JSON Schema dicts); only `cache_control` is genuinely Anthropic-specific, and it remains the right seam for the current single-vendor implementation. (d) The interface change should land **with** the second vendor, not before, so `StructuredOutputOptions` can be designed against the actual divergence (cache markers vs. OpenAI's prompt-prefix `cache_control: "auto"` vs. Gemini's implicit caching) instead of speculating. Trigger: first non-Anthropic `ICoachingLlm` implementation lands (Haiku via the tiered router counts since it shares the Anthropic shape). |
| 2026-04-27 | Slice 1 (PR #67 review carry-forward) | CodeRabbit (#141, inline 3146847070, marked Critical) flagged a check-then-write race on `IIdempotencyStore`: two concurrent same-key submissions can both miss `SeenAsync`, both run the 6-call macro/meso/micro LLM chain, then both `Record` — proposing a first-writer-wins reservation API (`TryBeginAsync` / `CompleteAsync`) backed by an atomic insert-or-fail. | **Deferred — DEC-057 already specifies the structural fix (`opts.Policies.OnException<Marten.Exceptions.ConcurrencyException>().MoveToErrorQueue()`) so concurrent submissions surface as 409 Conflict rather than silently re-running 6 LLM calls; that policy is currently NOT wired in `Program.cs` `UseWolverine` despite being named in DEC-057's "What shipped in Slice 1 spec" bullet (decision-log line 1818). The right structural follow-up is to land the DEC-057 policy + flip `MartenConfiguration` from `EventAppendMode.Quick` to `Strict` for the onboarding/plan streams (or re-decorate `OnboardingTurnHandler.Handle` as `[AggregateHandler]` with an `IEventStream<T>` parameter so Marten's optimistic-concurrency check fires on `Append`), not to stack a second reservation primitive on top of the idempotency document. CodeRabbit's reservation API would duplicate the concurrency seam Marten already owns and bypass the canonical 2026 single-handler / single-session / single-transaction pattern.** First-turn race is already covered: `session.Events.StartStream<OnboardingAggregate>(streamId, …)` enforces stream-id uniqueness, so the second concurrent first-turn commit fails with `ExistingStreamIdCollisionException`. The exposure is narrow (subsequent-turn appends under `EventAppendMode.Quick` and the missing exception policy) and the operational impact at MVP-0 single-user scale is wasted LLM tokens + a duplicate transcript-event pair on the rare client-retry-before-original-responds window — not data corruption that crosses tenants or breaks the projection invariants. Triggers to implement now: (T1) DEC-057 policy + `EventAppendMode` decision lands as its own task before Slice 4's open-conversation endpoint multiplies the call surface; (T2) telemetry (once added) shows duplicate-key acceptance > 0.1% of submissions in any 7-day window; (T3) any post-MVP-0 multi-device or background-retry workflow that broadens the concurrent-submit window beyond a single user-agent. Linked deferral, scoped to the same DEC-057 follow-through. |
