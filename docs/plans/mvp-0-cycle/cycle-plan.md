...
- **Decisions locked for Slice 3 (2026-06-07 brainstorm + verification):** **DEC-078** (adaptation triggering = deterministic DEC-012 gate + simplified MVP-0 signal; L0 absorb / L1 nudge handled in code with no LLM, L2 restructure = first LLM call; per-log band-deviation + hysteresis; full EWMA/ACWR/CTL/ATL/TSB load model deferred — **resolves the pragmatic-default-vs-DEC-012 conflict in favor of DEC-012**), **DEC-079** (high-risk safety subset — crisis 988/741741 short-circuit + emergency-referral + Amber injury/RED-S, full DEC-030 taxonomy deferred to pre-public-release; net-new Marten-projected `ConversationTurn` read-model for atomicity; Pattern-B `PlanAdaptationOutput`; `WorkoutLogId`-keyed adaptation idempotency; read-only Explain-the-change chat panel; blocking, no streaming). Inherits DEC-012 / DEC-058 / DEC-060 / DEC-073 / DEC-019 / DEC-030 / DEC-027 verbatim. **Verification (4 agents vs codebase + decision log) corrected three load-bearing brief claims now baked into the spec:** DEC-073's exception types + `Kind=Error` envelope are net-new (own PR4), the prescription snapshot is a double-precision pace *band* (not int scalar), and `MicroWorkoutsByWeek` holds only week 1 (adaptation edits the current micro week + meso weekly targets, not future micro detail). User audience steer: MVP-0 is self + family for the foreseeable future — prioritize a working loop + clean UX over safety breadth. **DEC-080 ratified in PR5 (#169, 2026-06-10):** two deliberate MVP-0 adaptation-orchestration scope reductions surfaced by the PR5 deep review — validator-rejected LLM proposals are terminal with zero re-prompts (the local spec's "one re-prompt on validator reject" is **not** implemented; DEC-073 governs only transport-level SDK retries, and a constrained-decoding reject on a frozen schema is unlikely to be fixed by an identical second call), and `ComposeForAdaptationAsync` omits the runner-profile block (the PR4 `adaptation.v1.yaml` template defines no profile token, so adding it is a template + eval-cache re-record change). Both revisited at Unit 6 eval calibration against the five `TestProfiles`.
- **Next Step:** **Implement Slice 3 PR6 (eval + DEC-074 sentinel).** PR1 (deterministic SafetyGate, #161), PR3 (events/projection, #163), and PR2 (deviation engine, #165) merged 2026-06-08; PR4 (adaptation output/LLM/DEC-073, #167) merged 2026-06-09; PR5 (orchestration — the single integration gate, #169) merged 2026-06-10. The spec + PR plan are written (`docs/specs/17-spec-slice-3-adaptation/`). 7 dependency-ordered PRs: **PR1 SafetyGate ✅ (#161) → PR3 events/projection ✅ (#163) → PR2 deviation engine ✅ (#165) → PR4 adaptation output/LLM/DEC-073 ✅ (#167) → PR5 orchestration ✅ (#169) → PR6 eval+DEC-074 sentinel → PR7 read-only chat panel**. Generate the task graph from the spec, then build PR6 next. The loop is now wired end-to-end through PR5: logged workout → deterministic deviation + escalation gated by the deterministic SafetyGate → Pattern-B structured plan modification → `PlanAdaptedFromLog` appended to the Marten plan stream → `PlanProjection` updates → plan re-renders → a Marten-projected `ConversationTurn` surfaces the explanation in the read-only Explain-the-change panel. PR6 records the eval cache + the DEC-074 prompt↔cache SHA-256 sentinel — the first real-LLM surface in the slice — and Slice 3 gates the **live end-to-end validation pass** (`ROADMAP.md` § MVP Milestones) once the onboard → plan → log → adapt loop is whole.
- **Blockers:** None.
...
| 2026-06-09 | Slice 3 (PR4 #167 CI) | `OnboardingTurnConcurrencyIntegrationTests` (the DEC-057 `EventAppendMode.Rich` stream-collision regression guard) flaked once under the coverage-instrumented `Backend analysis` CI job with `successCount == 2` — two concurrent `StartStream<OnboardingView>` commits winning instead of the asserted one. This is a **violation of the exactly-one-winner correctness property**, not a failure to reproduce the race, so it is either **(a)** a genuine rare gap in the Rich-mode stream-collision guarantee (intermittent duplicate-onboarding-stream corruption — the exact DEC-057 scenario) or **(b)** a test-harness contention artifact widened by coverage-instrumentation timing. Passed locally (1510/1510), the normal `Backend (build + test)` job, and the re-run. Not introduced by PR4 (no onboarding/concurrency files touched). | **Scheduled — dedicated root-cause investigation before the Slice 3 live end-to-end validation pass (`ROADMAP.md` § MVP Milestones), independent of the PR5 → PR7 train.** Reproduce under stress (loop the test / raise `ConcurrentSubmitCount` / run under coverage locally) to confirm recurrence; inspect the SQL Marten emits for `StartStream<OnboardingView>` under Rich mode and whether `mt_streams`' PK is the sole guard; then **(a)** fix the concurrency guarantee or **(b)** make the N-way contention deterministic (a start barrier so all sessions reach `SaveChangesAsync` together). **Must not be masked with a retry / rerun / loosened assertion** — a real (a) is silent data corruption. |
| 2026-06-10 | Slice 3 (PR5 #169) | Three follow-ups from the integration-gate PR + its deep review: **(a)** the global DEC-057 concurrency rules in `Program.cs` rest on the same disproved exception type (`Marten.Exceptions.ConcurrentUpdateException`) that PR5's suite proved wrong for Rich-mode append races — a real onboarding/regenerate-chain append race would match no global rule and fall into Wolverine's default retry loop, defeating first-write-wins; **(b)** adaptation idempotency markers expire after 48h (sweeper) while the create-side EF idempotency is permanent, so a re-trigger >48h after the original adaptation would re-evaluate; **(c)** only `Weather`/`Terrain` free-text metric values route through the DEC-059 sanitizer, so a string value smuggled under a numeric metric key (e.g. `hrAvg`) reaches the adaptation prompt's nonce-spotlighted recent-logs section having bypassed the PI pattern catalog — only `EscapeDelimiterBody` applies. | **(a) folded into the scheduled #168 flake investigation (row above); (b) deferred to a pre-Slice-4 guard decision; (c) promoted to `docs/features/backlog.md`.** (a) PR5 deliberately left `Program.cs` untouched — it is a plausible root-cause lead for the `OnboardingTurnConcurrencyIntegrationTests` two-winners flake and belongs in that root-cause pass, not a drive-by edit. (b) Mostly theoretical under synchronous post-create dispatch (the create path is permanently idempotent, so a duplicate create can't re-trigger inside 48h); worth a guard decision before async dispatch arrives in Slice 4. (c) Delimiter-escaping + the spotlight nonce still prevent section break-out, so this is a defense-in-depth gap, not a full break, and the MVP-0 audience is self + family; the structural fix is to validate the metrics bag at the create boundary (numeric canonical keys reject non-numeric JSON; reject keys outside `WorkoutMetricKeys.All`) or route every rendered metric value through the DEC-059 sanitizer. PR5's own deep-review + CodeRabbit findings landed in-PR (the suite-found `EventStreamUnexpectedMaxEventIdException` retarget, the validating `AdaptationSignalState.Create` factory, the dead-zone re-escalation lock, and the DEC-080 scope record); no other carry-forward. |
