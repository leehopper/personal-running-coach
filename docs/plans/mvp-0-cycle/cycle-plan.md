...
- **Active Slice:** Slice 0 (Foundation) — Unit 1 persistence substrate shipped (PR #49 merged 2026-04-21, commit `46348fc`; all 14 CI checks green on merge — Backend build+test, Frontend build+test, CodeQL csharp + javascript-typescript, SonarCloud backend + frontend, License & dependency review, Trivy, Backend analysis, Frontend analysis, CI Gate, Detect changes, submit-nuget). DEC-048 composition corrections, DEC-049 (R-055 resolution), and the 2026-04-21 deep-review + CodeRabbit follow-up pass (scoped fixture env-var save/restore, `StartupSmokeIntegrationTests` rename with AAA markers + DevelopmentMigrationService registration coverage + FluentAssertions `.Which` chain, DRY `InitializeAsync`, `SmokeTests.cs` retired, `DpKeysContext` DbSet accessor, docker-compose.otel.yml jaeger healthcheck, spec lines 47/61/170 aligned to DEC-049 runtime envelope provisioning) all landed with the merge. Full suite **582 passing / 0 failing / 0 skipped** locally.
- **Active Slice Spec:** `docs/specs/12-spec-slice-0-foundation/`
- **Next Step:** Begin Unit 2 (T02.x — Auth API) starting with T02.1 (JwtBearer package + Identity Core registration + password policy + `CookieOrBearer` name), then T02.2 → T02.5 (auth schemes + antiforgery + middleware, global exception handler, `AuthController`, integration-test matrix). After Unit 2 lands, Unit 3 (T03.x — frontend auth UX) closes out Slice 0. Slice 1 requirements doc (`./slice-1-onboarding.md`) was amended with R-048 / DEC-047 integration 2026-04-19.
- **Blockers:** None.
...
| 2026-04-21 | Slice 0 (post-integration review — T02.4 blockers) | Two genuine unknowns surfaced while looking ahead at T02.4's `AuthController`: (a) the canonical ASP.NET Core 10 pattern for translating `IdentityResult.Errors` into `ValidationProblemDetails` keyed by DTO property name rather than `IdentityError.Code` — the spec mandates the DTO-property shape but the mechanics aren't obvious (R-058); (b) whether `SignInManager.PasswordSignInAsync` in .NET 10 is timing-safe on the unknown-email path or requires a manual dummy-hash mitigation — the spec's "or" between the two options signaled uncertainty (R-059, security-sensitive). | Research prompts at `docs/research/prompts/batch-19c-identity-result-to-validationproblemdetails.md` + `batch-19d-signinmanager-timing-safety.md`; queue rows R-058 + R-059 added. Both block T02.4 implementation — R-058 shapes the controller-to-SPA error contract; R-059 determines the login-action shape and has direct security implications. Do NOT begin T02.4 before both artifacts land and are integrated. |
