...
- **Active Slice:** **Slice 2b (Workout Logging proper) — in progress.** PR1 `plan-anchor` — the `PlanStartDate` calendar anchor on `PlanGenerated`/`PlanProjectionDto` + the pure `PlanCalendar` date→slot mapper (DEC-076 Unit 1) — merged 2026-06-01 as **#134**; PR6a `fe-infra` — the date-derived `resolveCurrentWeek` (replacing the lowest-populated-week heuristic that pinned home to week 1), the app-wide `<Toaster>` mount, and `FormMessage` `role="alert"` — merged 2026-06-01 as **#136**; PR2 `workoutlog-entity` — the `WorkoutLog` EF entity + EF-10 optional complex-type prescription snapshot + `WorkoutMetricKeys` + the repo's first value converters + migration + repository (DEC-072/DEC-076 Unit 2) — merged 2026-06-05 as **#145**; PR3 `create` — the `POST /api/v1/workouts/logs` create endpoint (server-authoritative snapshot resolution over the active plan + EF-native idempotency, DEC-077 Unit 3) — merged 2026-06-06 as **#147**; PR4 `query` — the DB-driven `POST /api/v1/workouts/logs/query` keyset history endpoint (newest-first, opaque keyset cursor, server-clamped page size, with sort/page/limit all in a single SQL query; DEC-076 § C Unit 4) — merged 2026-06-06 as **#151**; PR5 `context-assembler` — the `ContextAssembler` extension injecting recent logged workouts' notes + metrics as compact Layer-1 one-liners (shared `WorkoutMetricKeys.Metadata` + `RecentLogFormatter`, never collapsed in the overflow cascade, with an eval proof; DEC-076 Unit 5) — merged 2026-06-06 as **#154**; PR6b `log-form` — the `/log` route logging form + today's-card "Log" action (RHF/Zod with the schema derived from the generated create-request Zod, pessimistic create + success toast; DEC-075/DEC-076 Unit 6) — merged 2026-06-07 as **#156**. 7 of 8 stacked PRs merged; next is **PR7** (the frontend workout-history surface + E2E). Slice 2 (Workout Logging) was decomposed 2026-05-18 into two sub-projects, each with its own brainstorm → spec → implementation cycle: **(2a) Frontend Visual Foundation** and **(2b) Workout Logging proper**. **2a is complete and merged 2026-05-30** (see the next bullet). Slices 0, 1, and 1B are also complete and merged.
...
- **Active Slice Spec:** Sub-project 2b — `docs/specs/16-spec-slice-2b-workout-logging/` (gitignored / working-tree-only): spec + 7 Gherkin `.feature` files + `pr-strategy.md`, signed off and under implementation (PR1 + PR6a merged 2026-06-01; PR2 `workoutlog-entity` merged 2026-06-05; PR3 `create` merged 2026-06-06 as #147; PR4 `query` merged 2026-06-06 as #151; PR5 `context-assembler` merged 2026-06-06 as #154; PR6b `log-form` merged 2026-06-07 as #156). Sub-project 2a's spec (`docs/specs/15-spec-slice-2a-frontend-foundation/`) is fully implemented and merged; 2b's requirements remain at `docs/plans/mvp-0-cycle/slice-2-logging.md`.
...
- **Next Step:** **Continue Slice 2b** per the signed-off 8-PR strategy (`docs/specs/16-spec-slice-2b-workout-logging/pr-strategy.md`) — **PR1 `plan-anchor` ✅ merged 2026-06-01 (#134)**, **PR6a `fe-infra` ✅ merged 2026-06-01 (#136)** (date-derived `resolveCurrentWeek` fix, app-wide `<Toaster>` mount, `FormMessage` `role="alert"`), and **PR2 `workoutlog-entity` ✅ merged 2026-06-05 (#145)** (the `WorkoutLog` EF entity + EF-10 optional complex-type prescription snapshot + `WorkoutMetricKeys` + the repo's first `ValueConverter`s + migration + repository, DEC-072/DEC-076 Unit 2), and **PR3 `create` ✅ merged 2026-06-06 (#147)** (the `POST /api/v1/workouts/logs` create endpoint — server-authoritative snapshot resolution over the active plan + EF-native idempotency on a unique `(UserId, IdempotencyKey)` index, DEC-077 Unit 3), and **PR4 `query` ✅ merged 2026-06-06 (#151)** (the DB-driven `POST /api/v1/workouts/logs/query` keyset history endpoint — newest-first, opaque keyset cursor, server-clamped page size, with sort/page/limit all executing in a single SQL query, DEC-076 § C Unit 4), and **PR5 `context-assembler` ✅ merged 2026-06-06 (#154)** (the `ContextAssembler` extension injecting recent logged workouts' notes + metrics as compact Layer-1 one-liners — shared `WorkoutMetricKeys.Metadata` + `RecentLogFormatter`, never collapsed in the overflow cascade — with an eval proof, DEC-076 Unit 5), and **PR6b `log-form` ✅ merged 2026-06-07 (#156)** (the `/log` route logging form + today's-card "Log" action — RHF/Zod with the schema derived from the generated create-request Zod, pessimistic create + success toast, DEC-075/DEC-076 Unit 6); remaining order: PR7 history+E2E (next: **PR7** — the frontend workout-history surface, an ISO-week-grouped sparse `<dl>` list over the `POST .../query` endpoint, plus the slice's Playwright E2E coverage, Unit 7), stacked via squash + auto-merge + rebase-onto. The 2b spec (`docs/specs/16-spec-slice-2b-workout-logging/`, gitignored) + 7 Gherkin `.feature` files + PR strategy are written and signed off. Technical open items + all three carry-forwards were pre-resolved by Batch 28 (DEC-072–075; `slice-2-logging.md` § Research resolutions); the **2b product/UX brainstorm (2026-05-31)** resolved the four open questions in `slice-2-logging.md` § Brainstorm resolutions: (A) **snapshot-on-log** `WorkoutLog`↔prescription (server-authoritative nullable prescription snapshot capturing `SourcePlanId` + `(WeekNumber, DayOfWeek)` + a frozen prescribed-workout copy, persisted as an **EF Core 10 nullable complex type**; off-plan = null; no minted workout id; **server-side** date→slot matching via a new `PlanStartDate` plan anchor; supersedes the stale cycle-plan `PlannedWorkoutId` data-model line, and splits `LoggedAt` into `OccurredOn` + `CreatedOn`; recorded as **DEC-076**), (B) dedicated **`/log` route** + success toast (no preview surface — coaching framing is Slice 3), (C) **DB-driven `POST .../query`** keyset history endpoint (sort/filter/page all at the DB layer) with ISO-week-grouped client-side display, (D) **raw** history (UI sparse `<dl>` per DEC-075) + a `ContextAssembler` extension injecting compact layer-1 one-liners with explicit absence markers for RPE/HR only.
- **Blockers:** None.
