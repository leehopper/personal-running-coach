...
- **Active Slice Spec:** none yet. Sub-project 2a's design doc is `docs/plans/mvp-0-cycle/slice-2a-frontend-foundation.md`; R-075 has landed (integrated as DEC-070), so the 2a spec is the next step. Sub-project 2b's requirements are at `docs/plans/mvp-0-cycle/slice-2-logging.md`; its spec is written fresh after 2a ships.
...
- **Next Step:** Finish the sub-project 2a (Frontend Visual Foundation) brainstorm → design doc → research prompts (component library, Tailwind v4 theming) → spec → implement in small MRs. Then brainstorm sub-project 2b (Workout Logging).
- **Blockers:** None.
