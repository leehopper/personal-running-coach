...
- **Decisions locked for Slice 1B (pre-spec):** DEC-066 (OpenAPI codegen: `@rtk-query/codegen-openapi` + Orval v8 Zod, committed `backend/openapi/swagger.json`, `git diff --exit-code` drift gate, per R-071), DEC-067 (Marten upcasting: versioned CLR event types + `Events.Upcast<TOld, TNew>` + `MapEventTypeWithSchemaVersion<T>(N)`, per R-072).
- **Next Step:** Write the Slice 1B spec under `docs/specs/` from the requirements doc + DEC-066 / DEC-067, then drive implementation. Spec session reads `slice-1b-hardening.md` + `docs/research/artifacts/batch-24a-openapi-typescript-zod-codegen.md` + `docs/research/artifacts/batch-24b-marten-event-upcasting-strategy.md` + DEC-066/DEC-067. Slice 2 implementation does NOT start until Slice 1B merges.
- **Blockers:** None. R-071 and R-072 artifacts landed 2026-05-11; both queue entries marked Done. Slice 1B spec-writing is the unblocked next step.
