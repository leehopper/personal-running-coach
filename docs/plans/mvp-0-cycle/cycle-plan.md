...
- **Active Slice:** **Slice 2b (Workout Logging proper) — in progress.** PR1 `plan-anchor` — the `PlanStartDate` calendar anchor on `PlanGenerated`/`PlanProjectionDto` + the pure `PlanCalendar` date→slot mapper (DEC-076 Unit 1) — merged 2026-06-01 as **#134**; 1 of 8 stacked PRs done, next is **PR6a** (FE infra). Slice 2 (Workout Logging) was decomposed 2026-05-18 into two sub-projects, each with its own brainstorm → spec → implementation cycle: **(2a) Frontend Visual Foundation** and **(2b) Workout Logging proper**. **2a is complete and merged 2026-05-30** (see the next bullet). Slices 0, 1, and 1B are also complete and merged.
...
- **Active Slice Spec:** Sub-project 2b — `docs/specs/16-spec-slice-2b-workout-logging/` (gitignored / working-tree-only): spec + 7 Gherkin `.feature` files + `pr-strategy.md`, signed off and under implementation (PR1 merged 2026-06-01). Sub-project 2a's spec (`docs/specs/15-spec-slice-2a-frontend-foundation/`) is fully implemented and merged; 2b's requirements remain at `docs/plans/mvp-0-cycle/slice-2-logging.md`.
