...
- **Active Slice:** Slice 1 (Onboarding → Plan) — next. Slice 0 (Foundation) closed 2026-04-23 with PR #63 (Unit 3 Frontend Auth UX) on top of PR #49 (Unit 1 persistence substrate) and PR #50 (Unit 2 Auth API). End-to-end register → login → protected home → logout runs against a real browser under `__Host-` cookies + antiforgery. DEC-048 through DEC-056 captured the substrate decisions made along the way.
- **Active Slice Requirements:** `./slice-1-onboarding.md` (already amended with R-048 / DEC-047 lock-ins).
- **Active Slice Spec:** not yet written — fresh spec session writes it under `docs/specs/`.
- **Next Step:** Write the Slice 1 spec. Review or kick off Batch 17 research (R-051 LLM observability, R-052 Anthropic SDK choice, R-053 multi-turn eval pattern) before opening the spec session — all three target Slice 1 LLM call sites.
- **Blockers:** None.
...
| 2026-04-19 | Slice 0 (Batch 17 audit) | Batch 17 research queued (R-051 LLM observability, R-052 Anthropic SDK choice, R-053 multi-turn eval pattern) — targets Slice 1 LLM call sites. | Slice 1 spec session awaits the three artifacts. Prompts at `docs/research/prompts/batch-17{a,b,c}-*.md`. |
