...
- **Next Step:** **Fix the two sharp live-pass findings (the trademark leak in the live macro `Rationale` prose; the Amber safety referral dropped on absorb paths — both § Captured During Cycle, 2026-06-11 rows), then open Slice 4 (Open Conversation) with its brainstorm → spec session.** Slice 3 closed 2026-06-11: the gated live end-to-end validation pass ran against the Path B host-run stack + a funded Anthropic key and **PASSED** (see "Slice 3 — live end-to-end validation pass" below). Of the remaining findings, the race-date/plan-length fix follows before MVP-0 close, the onboarding Schedule-loop fix is bundled into Slice 4, and the restructure rationale↔patch consistency validator joins the DEC-080 Unit-6 eval-calibration revisit bucket. The full live pass re-runs at MVP-0 close after Slice 4. PR1 (deterministic SafetyGate, #161), PR3 (events/projection, #163), and PR2 (deviation engine, #165) merged 2026-06-08; PR4 (adaptation output/LLM/DEC-073, #167) merged 2026-06-09; PR5 (orchestration, the single integration gate, #169) and PR6 (eval suite + DEC-074 sentinel, #172) merged 2026-06-10; PR7 (read-only chat panel, #174) merged 2026-06-11. The spec + PR plan are written (`docs/specs/17-spec-slice-3-adaptation/`). 7 dependency-ordered PRs: **PR1 SafetyGate ✅ (#161) → PR3 events/projection ✅ (#163) → PR2 deviation engine ✅ (#165) → PR4 adaptation output/LLM/DEC-073 ✅ (#167) → PR5 orchestration ✅ (#169) → PR6 eval+DEC-074 sentinel ✅ (#172) → PR7 read-only chat panel ✅ (#174)**. The loop: logged workout → deterministic deviation + escalation gated by the deterministic SafetyGate → Pattern-B structured plan modification → `PlanAdaptedFromLog` appended to the Marten plan stream → `PlanProjection` updates → plan re-renders → a Marten-projected `ConversationTurn` surfaces the explanation in the read-only Explain-the-change panel. The live pass's precondition — the flaky-concurrency-test investigation — was resolved 2026-06-11 (test-harness timing artifact, not a Marten Rich-mode gap; the stream-collision test is now deterministic, PR #182 — § Captured During Cycle), and the pass itself ran the same day and PASSED.
- **Blockers:** None.
...
### Slice 3 — live end-to-end validation pass (2026-06-11)

The gated live pass (`ROADMAP.md` § MVP Milestones) ran 2026-06-11 against the Path B host-run stack (Postgres + Redis via compose, host `dotnet run` API, Vite) with `Anthropic:ApiKey` from the `runcoach-api` user-secrets store pointed at a funded account — the first non-Replay exercise of the full coaching path. **The loop PASSED:**

- [x] register → onboarding (live Sonnet turns) → plan generation (~2.5 min; 16-week macro/meso/micro rendered on home).
- [x] 3 progressively under-performing logs walked the escalation ladder exactly as designed: score 1.0 (L0, silent) → 2.0 (score-driven L1, absorbed by design, no event) → 3.0 → **live L2 restructure** — Coach-panel turn + `BeforeAfterDiff`, meso weekly targets 30/32/36/30 → 24/26/30/26 km, plan re-rendered in the same interaction, panel survives reload.
- [x] A 4th under-performing log probed the post-restructure dead-zone: "would-be Restructure … cooldown active: True" — suppressed correctly per the hysteresis design.
- [x] Test account `live-pass-20260611@runcoach.test` persists in the dev Postgres volume with the adapted plan (useful for poking at the panel). Screenshots live with the slice spec artifacts (`docs/specs/17-spec-slice-3-adaptation/*.png`, working-tree-only).

The pass surfaced **5 findings — none blocking the loop itself** — triaged into § Captured During Cycle (rows dated 2026-06-11): the trademark leak in the live macro `Rationale` prose and the Amber-referral-dropped-on-absorb safety gap (both scheduled as immediate fixes), the race-date/plan-length mismatch (fix before MVP-0 close), the onboarding Schedule-step loop (bundled into Slice 4), and the restructure rationale↔patch consistency gap (DEC-080 Unit-6 eval-calibration revisit).
...
| 2026-06-11 | Slice 3 (live e2e validation pass) | **Trademark leak at the live surface** — the `plan_generated_v1` event's `Macro.Rationale` prose contains "Using Daniels' Running Formula, your VDOT sits around 38", rendered to the user on the plan surface. The trademark guard covers the assembled prompt + the adaptation outputs (PR6 eval guard), not the macro-plan structured output's prose fields. | **Scheduled — immediate fix (next PR, paired with the Amber-referral fix).** Fix shape: terminology instruction in the macro-plan prompt (Daniels-Gilbert / "pace-zone index" wording per `CLAUDE.md` § Trademark Rule) + extend the eval trademark guard to every LLM-authored prose field that reaches the user (macro/meso rationales; adaptation rationale is already guarded). Prompt edit ⇒ DEC-074 manifest regen + eval-cache re-record against the funded key. |
| 2026-06-11 | Slice 3 (live e2e validation pass) | **Amber safety referral dropped on absorb paths** — `EvaluateAdaptationHandler` appends the Amber `SafetySignalRaised` referral only inside `RestructureAsync` (only the Red short-circuit is handled top-level), so Amber + L0/L1 absorb — including the post-restructure cooldown dead-zone — returns with no safety turn. Live repro: "sharp pain in my left shin" note (SG-I01) logged during cooldown → silence in the panel. Claim re-verified against the handler source 2026-06-11. | **Scheduled — immediate fix (next PR).** DEC-079's intent is referral-on-Amber regardless of plan action: hoist the Amber referral append out of `RestructureAsync` so it fires at any escalation level, with integration coverage per path (L0 / L1 / dead-zone / L2). |
| 2026-06-11 | Slice 3 (live e2e validation pass) | **Plan length ignores race date** — macro generation produced `TotalWeeks=16` for a 10K declared 9 weeks away (2026-08-15); the race lands mid-Peak and taper weeks 14–16 fall after the race. | **Scheduled — fix before MVP-0 close (own small PR after the sharp pair).** Fix shape: derive/constrain plan length from weeks-until-race in the macro prompt **plus** a deterministic post-deserialization check (`TotalWeeks` vs. the race-date window — structural enforcement, not `[Description]` hints, per the DEC-042 lesson and the open "structured output post-deserialization validation" deferred item in `ROADMAP.md`). Prompt edit ⇒ eval-cache re-record. |
| 2026-06-11 | Slice 3 (live e2e validation pass) | **Onboarding Schedule step loops on free-text** — day/duration answers given in free text weren't merged into the Schedule slots (`clarification_requested` claimed session duration was missing although it had been given twice; 4 round-trips until one message phrased both slots explicitly). Secondary: input-kind/text mismatch — the structured day-picker rendered while the assistant text asked about fitness-feel. | **Deferred — bundled into Slice 4 (open conversation).** Slice 4 re-touches the turn engine + prompts; fold the slot-merge prompt fix and an input-kind ↔ assistant-text consistency check into that work. If Slice 4's scope ends up not reshaping onboarding turns, fix as a standalone prompt iteration at MVP-0 close (the full live pass re-runs then anyway). |
| 2026-06-11 | Slice 3 (live e2e validation pass) | **Restructure rationale/patch mismatch** — the rationale claimed "Thursday becomes a short gentle recovery run" but the diff contains no Thursday change; the new week-1 weekly target (24 km) equals Mon+Wed+Sat only (4+8+12) while the untouched Thursday 6 km still sits in the micro week (sum 30 km); Saturday distance went **up** 11→12 km in a fatigue restructure. `PlanAdaptationOutputValidator` enforces neither micro-week-sum ↔ weekly-target consistency nor rationale ↔ patch correspondence. | **Deferred — joins the DEC-080 Unit-6 eval-calibration revisit bucket.** Fix shape: a deterministic sum-consistency validator rule (current micro-week workout sum must equal that week's meso target) + eval scenarios over the cached restructures; rationale ↔ patch correspondence is LLM-judge territory — calibrate it with the Haiku communication judge at the same revisit. |
...
- [x] …log a workout that deviates meaningfully from plan (e.g., distance way off, or freeform notes indicating walking/injury/external factor). *(Live pass 2026-06-11: 3 under-performing logs + a dead-zone probe.)*
- [x] …see the plan adjust in response, with the event stored in the Marten stream. *(Live L2 restructure: `PlanAdaptedFromLog` appended, weekly targets 30/32/36/30 → 24/26/30/26 km, plan re-rendered same interaction.)*
- [x] …see the coach's explanation ("I adjusted your plan because…") appear in the chat panel. *(Coach-panel turn + `BeforeAfterDiff`; survives reload.)*
- [x] …verify via eval that adaptation handles the absorb/nudge/restructure cases correctly per DEC-012's escalation ladder (at least levels 1-3). *(PR6 #172: 20 deterministic classification scenarios, zero hard fails; safety pass-rate 1.0.)*
