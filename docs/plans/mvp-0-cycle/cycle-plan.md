...
- **Next Step:** Brainstorm sub-project **2b (Workout Logging)** in a fresh session, then write the spec under `docs/specs/`. The technical open items and all three carry-forwards are pre-resolved by Batch 28 (DEC-072–075, summarized in `slice-2-logging.md` § Research resolutions); the brainstorm focuses on the remaining **product/UX** questions: `WorkoutLog`↔prescribed-workout relationship (FK vs date-match; off-plan logs), post-log flow, history pagination/filtering/sorting, and raw-logs-vs-layer-1-summaries in history.
- **Blockers:** None.
