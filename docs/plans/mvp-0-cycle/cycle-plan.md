...
| 2026-04-26 | Slice 1 (T117 / DEC-063 carry-forward) | xunit collection-parallelism is currently disabled assembly-wide (DEC-063); re-enabling requires per-collection database isolation in `RunCoachAppFactory`. | **Deferred** with explicit triggers (per DEC-063): suite duration > 30s OR additional integration test classes pushing wall-time. Implementation path: split the assembly fixture into per-collection fixtures with isolated Postgres schemas, document the trade-off vs SUT boot cost. |
