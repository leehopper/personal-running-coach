...
- **Active Slice Spec:** Sub-project 2b — `docs/specs/16-spec-slice-2b-workout-logging/` (gitignored / working-tree-only): spec + 7 Gherkin `.feature` files + `pr-strategy.md`, signed off and under implementation (PR1 + PR6a merged 2026-06-01). Sub-project 2a's spec (`docs/specs/15-spec-slice-2a-frontend-foundation/`) is fully implemented and merged; 2b's requirements remain at `docs/plans/mvp-0-cycle/slice-2-logging.md`.
...
| 2026-06-02 | Slice 2b (PR2 #138 multi-agent review) | (a) `WorkoutPrescriptionSnapshot` has no fast/slow pace-ordering construction guard — the computed `PrescribedPace` `PaceRange` view throws on inverted bounds, and only at read time. (b) `WorkoutLog.Splits` exposes a mutable `List<>` on a type documented as an immutable historical fact. | **Deferred to first consumer (PR3+).** (a) The server-authoritative construction site is PR3, which sources already-ordered plan paces (`TargetPaceFastSecPerKm ≤ TargetPaceEasySecPerKm`); add a validating static factory + unit test when PR3 builds the snapshot. (b) The whole EF entity is a conventionally-mutable POCO; only convert `Splits` to a read-only `IReadOnlyList` surface (private backing field + `PropertyAccessMode.Field`) if a consumer needs structural immutability. |
