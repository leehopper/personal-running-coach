...
- **Next Step:** **Implement Slice 3 PR4 (adaptation output/LLM/DEC-073).** PR1 (deterministic SafetyGate, #161), PR3 (events/projection, #163), and PR2 (deviation engine, #165) merged 2026-06-08. The spec + PR plan are written (`docs/specs/17-spec-slice-3-adaptation/`). 7 dependency-ordered PRs: **PR1 SafetyGate ✅ (#161) → PR3 events/projection ✅ (#163) → PR2 deviation engine ✅ (#165) → PR4 adaptation output/LLM/DEC-073 (first live) → PR5 orchestration (the single integration gate) → PR6 eval+DEC-074 sentinel → PR7 read-only chat panel** (PR4 may split into 4a/4b for a focused DEC-073 review). Generate the task graph from the spec, then build PR4 next. The loop: logged workout → deterministic deviation + escalation gated by the deterministic SafetyGate → Pattern-B structured plan modification → `PlanAdaptedFromLog` appended to the Marten plan stream → `PlanProjection` updates → plan re-renders → a Marten-projected `ConversationTurn` surfaces the explanation in the read-only Explain-the-change panel. Slice 3 also gates the **live end-to-end validation pass** (`ROADMAP.md` § MVP Milestones) once the onboard → plan → log → adapt loop is whole.
- **Blockers:** None.
