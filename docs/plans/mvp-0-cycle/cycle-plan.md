...
- **Active Slice:** Slice 0 (Foundation) — Unit 1 persistence substrate PR (#49) review round applied. DEC-048 composition corrections and DEC-049 (R-055 resolution) both landed 2026-04-20; 2026-04-21 deep-review + CodeRabbit follow-ups applied (scoped fixture env-var save/restore, `StartupSmokeIntegrationTests` rename with AAA markers + DevelopmentMigrationService registration coverage + FluentAssertions `.Which` chain, DRY `InitializeAsync`, `SmokeTests.cs` retired, `DpKeysContext` DbSet accessor, docker-compose.otel.yml jaeger healthcheck, spec lines 47/61/170 aligned to DEC-049 runtime envelope provisioning). Full suite **582 passing / 0 failing / 0 skipped** locally.
- **Active Slice Spec:** `docs/specs/12-spec-slice-0-foundation/`
- **Next Step:** Push review-round fixes to PR #49, confirm CI green on the Linux host runner + license-review, merge. Unit 2 (T02.x — Auth API) begins immediately after merge. Slice 1 requirements doc (`./slice-1-onboarding.md`) was amended with R-048 / DEC-047 integration 2026-04-19.
