...
- **Active Slice Spec:** `docs/specs/15-spec-slice-2a-frontend-foundation/` — written and fully implemented (16 tasks, four branches, 2026-05-19); awaiting push / PR merge / post-merge verification (see the 2a cleanup checklist below). Sub-project 2b's requirements are at `docs/plans/mvp-0-cycle/slice-2-logging.md`; its spec is written fresh after 2a ships.
...
- **Next Step:** Push the four `slice-2a-*` branches, open and merge the four stacked PRs, then work the **Slice 2a post-merge cleanup checklist** below. Once 2a is merged, brainstorm sub-project 2b (Workout Logging) — requirements at `docs/plans/mvp-0-cycle/slice-2-logging.md`.
- **Blockers:** None.
...
### Slice 2a — pending integration & post-merge cleanup

Slice 2a is implemented (2026-05-19) but **not yet merged**. Do not consider 2a done — and do not start 2b — until every box below is checked.

- [ ] Push the four branches: `slice-2a-foundation`, `slice-2a-contrast-gate`, `slice-2a-migration`, `slice-2a-home-settings` (the dev environment cannot push non-interactively — push from a terminal).
- [ ] Open four stacked PRs; run `/review-pr` on each; merge in dependency order: **foundation → {contrast-gate, migration} → home-settings**.
- [ ] `main` carries an unpushed commit `fbe80c4` (the R-075 integrate commit) — it rides the foundation PR; nothing extra to do, just expect it in PR1.
- [ ] **Run the Playwright E2E suite (`npm run e2e`) on `main` after merge** — it was not runnable in the worktrees (needs the .NET backend + Postgres up). The 5 onboarding E2E specs assert `data-testid` selectors that the colour-only migration preserved; confirm green.
- [ ] **Confirm `check-contrast` passes on `main`** — the script (T02, on `slice-2a-contrast-gate`) and the migrated surfaces (T03/T04) lived on separate branches, so the gate never ran against the fully integrated tree. It runs in pre-commit + CI once foundation + contrast-gate are merged.
- [ ] Remove the four `.worktrees/slice-2a-*` worktrees (`git worktree remove`) and delete the merged local branches.
- [ ] Reap the stray Vite dev server on port 5234 and the leftover blocked `git` processes from the failed non-interactive push attempt.
- [ ] Mark Slice 2a complete in this Status block and `ROADMAP.md`, and add a Cycle History row.
...
| 2026-05-18 | Slice 2 (pre-spec `/catchup` brainstorm) | Slice 2 decomposed into sub-project 2a (Frontend Visual Foundation) + 2b (Workout Logging proper), with 2a sequenced first. The frontend has no design foundation — bare `index.css`, hardcoded per-component colors, no dark mode, and shadcn/ui never installed despite the stack listing. | **In progress.** 2a covers palette + semantic tokens, light/dark, Tailwind v4 `@theme`, the component-library decision (subsumes the 2026-05-06 deferred shadcn-bootstrap item), typography/spacing scale, and migrating the existing surfaces onto the system. Calendar / chat-UI / animation libraries are explicitly excluded. The component-library + theming choices route through the research-prompt cycle. See the Status section's sub-project 2a bullet. |
