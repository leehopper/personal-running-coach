...
| Sanitize ALL rendered metric values, not just free-text keys (prompt-injection defense-in-depth) | `RecentLogSanitizer.FreeTextMetricKeys` routes only `Weather`/`Terrain` through the DEC-059 `IPromptSanitizer` pattern catalog. A client can submit a string value under a numeric metric key (e.g. `hrAvg`); `WorkoutMetricsProjection.ToDisplayMetrics` passes string values through verbatim, and `RecentLogFormatter` renders any `WorkoutMetricKeys.Metadata` key into the adaptation LLM prompt's nonce-spotlighted recent-logs section having bypassed the pattern sanitizer — only `EscapeDelimiterBody` applies. Delimiter-escaping + the spotlight nonce still prevent section break-out, so this is a defense-in-depth gap, not a full break-out, and the MVP-0 audience is self + family. Fix: validate the metrics bag at the create boundary (numeric canonical keys reject non-numeric JSON; reject keys outside `WorkoutMetricKeys.All`) OR route every rendered metric value through the DEC-059 sanitizer. Surfaced by the PR #169 deep review (confidence 80%); deferred there as outside PR5's diff. |
