...
5. **Log draft card** (`StructuredLogDraft` via `CoachCard`) — `LOG THIS RUN?` + 2×2 condensed value grid (DISTANCE/TIME/DATE/STATUS, status in moss) + `ON-PLAN — <workout>` mono line when prescription matched + CONFIRM (primary) / EDIT (secondary) / CANCEL (ghost). States: **partial parse** (deferred — unreachable this cycle, since the backend never emits partial `StructuredLogDraft`s; documented here for future reference only: missing value renders `—` + clay `MISSING` tag, CONFIRM disabled at 35%, EDIT emphasized with clay border); **saving** (buttons locked, card dims, `SAVING…`); **success** (card collapses to persistent one-line receipt: moss check + `LOGGED — 9.2 KM · 41:00 · JUL 8` + `LOG BOOK →`); **failure** (card stays open, error toast, retry reuses idempotency key — existing DEC-077 behavior).
