...
> **Correction note (2026-04-19):** This artifact recommended installing the Superpowers plugin as a foundational step. That recommendation did not hold up in practice — Superpowers' prescriptive "you MUST use this skill" framing pulled the project workflow in a direction that ran counter to the way this repo actually operates. Superpowers has been removed. Additionally, the project has stepped back from prescribing specific skills inside its own planning docs — pick the right skill for the task at runtime, don't encode the choice in durable docs. The cw-workflow directory conventions (`docs/specs/{NN}-spec-{topic}/…`) are retained as useful structure, but not as a required pipeline. The rest of this artifact is preserved as historical record of what the research said at the time; treat the specific plugin-install and skill-invocation recommendations below as superseded.
...
- **Superpowers** by obra (27,900 stars) — the most battle-tested skills library, covering TDD, debugging, collaboration patterns, and ~20 core engineering skills. This is the single best plugin to install first. **[SUPERSEDED 2026-04-19 — see correction note at top. Superpowers was installed and then removed; its prescriptive "you MUST use this skill" framing conflicted with how this project wants to operate.]**
