...
- **Registration shape** on `StoreOptions` consistent with DEC-062's `opts.Add(...)` discipline for EF projections and `opts.Projections.Add(...)` for document projections.
...
- **Gotchas** specific to conjoined tenancy (DEC-062), the dual-write atomicity rule (DEC-060), and Anthropic Pattern-B's byte-stable schema constraint (DEC-058).
