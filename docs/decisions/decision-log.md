...
1. Every RunCoach ASP.NET Core 10 host sets `DOTNET_hostBuilder__reloadConfigOnChange=false` before `WebApplication.CreateBuilder(args)` evaluates. `Program.cs` sets it as the first executable statement via `Environment.SetEnvironmentVariable`; `RunCoachAppFactory` re-asserts it via `builder.UseSetting(...)` as belt-and-suspenders.
2. `RunCoachDbContext.OnModelCreating` does NOT call `builder.MapWolverineEnvelopeStorage()`. Wolverine's `WolverineModelCustomizer` (registered via `AddDbContextWithWolverineIntegration<T>`) is the sole wiring path; the two call sites collided by double-adding the `WolverineEnabled` annotation.

**Rationale:**

- On macOS arm64 / Darwin 25.x, `FileSystemWatcher.StartRaisingEvents` synchronously calls `Interop.Sys.Sync()` (dotnet/runtime#77793), which stalls unboundedly. `CreateBuilder` installs three default-reloading watchers (appsettings, appsettings.Development, user-secrets) and hangs. Disabling host-config reload elides the watchers; `CreateBuilder` returns in ≈3 s.
- Config reload on change is a dev-loop nicety, not a runtime contract. CI and production do not edit config files in-place; `dotnet watch` handles dev-loop edits via host restart.
- Envelope entities stay in the runtime model via the customizer; migrations remain `ExcludeFromMigrations`-safe (Wolverine provisions the tables via `ApplyAllDatabaseChangesOnStartup`).
- Test suite: 575/0/1 (scope-reduced) → 581/0/1 with full SUT-host fixture + six smoke tests.

**Alternatives considered (and rejected):**

- Guard the env-var on `OperatingSystem.IsMacOS()` — over-engineering; reload-on-change isn't load-bearing anywhere.
- Drop `<UserSecretsId>` or `appsettings.Development.json` — partial fix; the other watchers still install.
- Switch SUT TFM to `net9.0` (R-047 escape hatch) — loses .NET 10 Darwin 25.x exposure; config-level fix lands cleanly.
- Guard `MapWolverineEnvelopeStorage` with `FindAnnotation` — customizer runs after `OnModelCreating`, so the guard never trips.

**Cross-references:**

- R-055 artifact: `docs/research/artifacts/batch-18b-webapplication-createbuilder-hang-followup.md`.
- DEC-048 — composition recipe this builds on.
- dotnet/runtime#77793 — `FileSystemWatcher` startup on macOS.
...
1. `app.UseHttpsRedirection()` is called unconditionally (no `!IsDevelopment()` gate). The middleware self-disables with a warning when it cannot resolve an HTTPS port.
2. `services.AddHttpsRedirection(o => o.RedirectStatusCode = Status307TemporaryRedirect)` is registered explicitly to pin 307 (not 308) — 308 is aggressively cached.
3. `__Host-RunCoach` application cookie sets `options.Cookie.Path = "/"` explicitly — RFC 6265bis §5.6 requires the attribute to be *present*, not merely defaulted, or Chrome / Safari silently drop the cookie.
4. `RunCoachAppFactory.ConfigureWebHost` calls `builder.UseSetting("https_port", "443")` to silence the `TestServer`'s `IServerAddressesFeature`-missing warning on every request.
5. Integration-test `HttpClient`s use `BaseAddress = https://localhost` + `AllowAutoRedirect = false` + `HandleCookies = false`. HTTPS base flips `Request.IsHttps = true` so the middleware short-circuits and `Secure` cookies emit; auto-redirect-off + no `CookieContainer` lets tests assert the raw `Set-Cookie` string.

**Rationale:**

- Ungated `UseHttpsRedirection` matches the .NET 10 template idiom; the middleware is designed to self-disable on unresolvable ports.
- `__Host-` without an explicit `Path` is a silent footgun — invisible in server logs, only caught in a real browser. Landing the explicit assignment before any browser-exercise test is necessary.
- `dev-certs https --trust` becomes a contributor prerequisite (captured in CONTRIBUTING.md); dev-only cookie weakening is rejected.

**Alternatives considered (and rejected):**

- Keep the `!IsDevelopment()` gate — defensible but non-idiomatic; adds cognitive load for zero correctness gain.
- Drop `__Host-` in Development — every significant .NET OSS project keeps cookie attributes stable across environments; divergence hides attack classes silently.
- `UseKestrel()` for integration tests — unnecessary for Set-Cookie assertions; reserve for Playwright E2E.
- mkcert as dev default — `dotnet dev-certs --trust` matches it on SANs + WSL + cross-platform; document mkcert as escape hatch only.
...
- dotnet/aspnetcore#27951 — `HttpsRedirectionMiddleware` unresolvable-port footgun.
...
1. `TokenValidationParameters` sets `ValidateIssuer`, `ValidateAudience`, `ValidateLifetime`, `ValidateIssuerSigningKey`, and `RequireSignedTokens` to `true` **unconditionally** — never gated on config presence. When `Auth:Jwt` is absent, `IssuerSigningKey` is `null` and every token fails closed (`IDX10500`).
2. `ValidAlgorithms = [SecurityAlgorithms.HmacSha256]` — closes the HS/RS key-confusion attack class.
3. `MapInboundClaims = false` — raw `sub` / `role` claim names, aligned with the .NET 8+ `JsonWebTokenHandler`.
4. HS256 is the issuance algorithm when the iOS shim lands. Single-trust-boundary topology (backend is the sole verifier); RS256 is over-application here.
5. `JwtAuthOptions` is a `sealed class` with `[Required]`/`[MinLength]` DataAnnotations; `JwtAuthOptionsValidator` is generated via `[OptionsValidator]` (reflection-free, AOT-safe).
6. `BindConfiguration(...).ValidateOnStart()` and the validator registration are both gated on `!IsDevelopment()`. Dev / CI tolerate absent config; Prod / Staging fail startup fast.

**Rationale:**

- Gating `Validate*` on config presence inverts "secure by default." A future `Authority = "..."` edit with JWKS auto-discovery would silently accept any token signed by that authority (RFC 8725 §2.1 / §3.9, OWASP JWT Cheat Sheet).
- `RequireSignedTokens = true` blocks `alg: none` independently of the signing-key validator — `JsonWebTokenHandler.ValidateJWSAsync` rejects unsigned tokens before key validation runs.
- `PolicyEvaluator` iterates every scheme in `policy.AuthenticationSchemes` without short-circuit. For `CookieOrBearer`, a bogus bearer header + valid cookie merges cleanly: Bearer fails silently, Cookie wins.
- Not `AddBearerToken` — that produces opaque Data-Protection tokens coupled to `MapIdentityApi`, not JWTs. iOS needs inspectable JWT claims.
- `ValidateOnStart` is hosted-service-driven (dotnet/aspnetcore#56453) — only fires on `IHost.StartAsync`, naturally skipped under `UseEnvironment("Development")`.

**Alternatives considered (and rejected):**

- Don't register `AddJwtBearer` until iOS needs it — pushes a Program.cs diff into the iOS PR; keeping `CookieOrBearer` shape-stable was a cycle-plan goal.
- Keep permissive validation — currently safe but latent landmine under future edits.
- RS256 with `RsaSecurityKey` — over-applied for single-trust-boundary; revisit if RunCoach federates.
- `NetDevPack.Security.Jwt` — out of scope for Slice 0; revisit if JWKS / rotation needed.

**Cross-references:**

- R-057 artifact: `docs/research/artifacts/batch-19b-jwtbearer-opt-in-registration.md`.
- DEC-033, DEC-044 — iOS deferred; browser uses cookies.
- RFC 8725; OWASP JWT Cheat Sheet; dotnet/aspnetcore#56453.
...
1. `AuthController.Register` translates a failed `IdentityResult` via a per-action loop: `foreach (var error in result.Errors) ModelState.AddModelError(propertyKey, error.Description);` → `return ValidationProblem(ModelState);`. Routes through `ProblemDetailsFactory.CreateValidationProblemDetails`; emits `application/problem+json`.
2. `RegisterRequest` DTO carries DataAnnotations (`[Required, EmailAddress, MaxLength(254)]`; `[Required, MinLength(12), MaxLength(128)]`) so malformed input short-circuits through `[ApiController]` auto-400. Identity remains source-of-truth for character-class rules and uniqueness.
3. `Modules/Identity/IdentityErrorCodeMapper.cs` maps the 22 stable `IdentityError.Code` values (stable across Identity 6–10 via `nameof(...)`) to DTO buckets (`password` / `email` / `username` / `role` / `general`) plus a `Kind` enum (`Validation` / `Conflict` / `Unauthorized` / `Unknown`).
4. `IdentityResultExtensions.ToRegistrationActionResult` splits the 409 conflict path (`DuplicateEmail` / `DuplicateUserName`) as plain `ProblemDetails` with a generic title; everything else routes through `ValidationProblem(ModelState)`.
5. Frontend parity: Zod schemas mirror the DataAnnotation rules in `shared-contracts` with a contract-test. FluentValidation deferred until rules become conditional.

**Rationale:**

- `ControllerBase.ValidationProblem(ModelStateDictionary)` is the canonical .NET 10 entry point — uniform shape with `[ApiController]` auto-400.
- **`CustomizeProblemDetails` does NOT fire for `ValidationProblem(ModelState)`** (dotnet/aspnetcore#62723). Centralizing via `AddProblemDetails` would be silently broken. Translation must live in the controller.
- `IdentityError.Code` values are invariant (produced by `nameof(...)` in `IdentityErrorDescriber`), so the mapper captures the full surface once; future codes fall through to the `General` default.
- DTO-property keying is a deliberate departure from `MapIdentityApi` (which keys by `IdentityError.Code`) — RunCoach's hand-rolled controller binds directly to React Hook Form / Zod conventions.
- 409 split: RFC 9110 §15.5.10 scopes 409 to conflicts. `DuplicateEmail` is a conflict, not a field-validation error; plain `ProblemDetails` prevents the SPA from branching on `errors.email` content.
- 409 `title` is deliberately generic (enumeration-resistance posture deferred to pre-public-release per spec Non-Goals).

**Alternatives considered (and rejected):**

- `IdentityFailureException` + dedicated handler — exceptions-for-control-flow is slower and harder to trace; .NET 10's `SuppressDiagnosticsCallback` also hides handled exceptions. Keep as a fallback for service-layer failures in later slices.
- `AddProblemDetails.CustomizeProblemDetails` callback — does not fire for MVC validation path.
- Middleware-based translator — reads body after the fact; brittle.
- `Result<T>` functional pattern — adds a dependency plus a `Result → IActionResult` translator; revisit when the service layer expands.

**Cross-references:**

- R-058 artifact: `docs/research/artifacts/batch-19c-identity-result-to-validationproblemdetails.md`.
- DEC-051 (sibling auth-contract decision), DEC-053 (timing-safety on login).
- dotnet/aspnetcore#62723; RFC 9110 §15.5.10; OWASP ASVS 5.0 V6.3.
...
1. `AuthController.Login` does NOT use `SignInManager.PasswordSignInAsync(string, ...)`. It calls `UserManager.FindByEmailAsync` first; on `user is null`, burns one `IPasswordHasher<ApplicationUser>.VerifyHashedPassword(new ApplicationUser(), DummyHash, request.Password)` pass to equalize timing, then returns a generic 401. On `user is not null`, delegates to `PasswordSignInAsync(user, password, isPersistent: true, lockoutOnFailure: false)`.
2. `DummyHash` is a `static readonly` field initialized once at type load via `new PasswordHasher<ApplicationUser>().HashPassword(...)`. Static caching avoids paying an extra PBKDF2 pass on every unknown-user request (which would flip the leak).
3. Every non-success `SignInResult` — `Failed`, `IsLockedOut`, `IsNotAllowed`, `RequiresTwoFactor` — collapses to an identical 401 `ProblemDetails` body. Distinctions are logged server-side but never surface in the response.
4. The 401 body is byte-identical across all failure causes. No conditional `detail`, `type`, or `extensions` members — any variance re-opens the enumeration channel on content and moots timing mitigation.
5. The mitigation is tagged `TODO` referencing this DEC so it can be retired cleanly if Microsoft ever ships built-in timing-safety in `SignInManager`.

**Rationale:**

- **`SignInManager.PasswordSignInAsync(string, ...)` in .NET 10 IS NOT timing-safe** on the unknown-user branch. Per `dotnet/aspnetcore` `src/Identity/Core/src/SignInManager.cs`: the string overload returns `Failed` immediately when `FindByNameAsync` returns null — no `VerifyHashedPassword` call. The user-found branch runs PBKDF2-HMAC-SHA512 / 100k iterations (~40–80 ms on server hardware) — a remotely observable delta.
- `VerifyHashedPassword(default, DummyHash, password)` is preferred over `HashPassword` because it traces the same code path as a real-user failure (parse V3 header → derive → constant-time compare), giving tighter parity at the same PBKDF2 cost.
- `IPasswordHasher<TUser>` exposes only synchronous operations — no `HashPasswordAsync` exists.
- Rate limiting is defense-in-depth, not a timing-signal mitigation. OWASP considers rate limiting alone insufficient.
- Collapse to 401, not 423 (WebDAV scope per RFC 4918 §11.3). Matches `MapIdentityApi`'s precedent.
- Test coverage is a code-path assertion (counting `IPasswordHasher` test double via `ConfigureTestServices`, one unknown-email + one wrong-password request, counter ≥ 1 in both). Wall-clock statistical benchmarks belong in the security-audit harness.

**Alternatives considered (and rejected):**

- Trust `PasswordSignInAsync` + rate-limit alone — rate limiting caps throughput, not signal.
- Custom `SignInManager` subclass — cleaner but tracks Microsoft's internal method changes; revisit at MVP-2 if login surfaces proliferate.
- Random-duration wrapper — masks without fixing; jitter window must exceed PBKDF2 delta to be effective, which degrades UX.
- Pre-hash password on client — schema change, incompatible with stock `PasswordHasher` and future WebAuthn.
- Sentinel user + `CheckPasswordSignInAsync` — `VerifyHashedPassword(user, null, password)` short-circuits.

**Known limitations (captured as MVP-1 follow-ups):**

- Lockout path re-opens the leak on a secondary axis (`AccessFailedAsync` DB write only on the known branch). Preferred mitigation: uniform-delay envelope wrapping the whole login action.
- Registration 409 `DuplicateEmail` is a separate enumeration vector (see DEC-052) — pre-public-release should migrate to "202 Accepted + email-to-existing-account."
- Password-reset endpoint must match login's posture (identical 200; fire-and-forget email across both branches).
- Do NOT add login-only server-side format checks; they create a third timing class. Keep validation in DataAnnotations.

**Cross-references:**

- R-059 artifact: `docs/research/artifacts/batch-19d-signinmanager-timing-safety.md`.
- DEC-052 — sibling error-contract decision.
- OWASP Authentication Cheat Sheet; ASVS 5.0 V6.3; WSTG §Account Enumeration.
- `dotnet/aspnetcore#54542` — related lockout-based enumeration.

---

## DEC-054: Post-research-integration auth-contract verifications — antiforgery SPA cookie, API-endpoint 401 default, antiforgery-attribute choice, `RequireUniqueEmail`

**Date:** 2026-04-21
**Status:** Partially superseded — bullet 3 is superseded by DEC-055.
**Category:** Auth substrate / Slice 0 / doc-verified corrections ahead of T02.4
**Informed by:** Microsoft Learn doc verification (`security/anti-request-forgery`, `security/authentication/api-endpoint-auth`) + R-058 / R-059 integration review.

**Decision:** Four verifications land ahead of T02.4:

1. **SPA-readable antiforgery cookie** (R-060). `/api/v1/auth/xsrf` writes `__Host-Xsrf-Request` alongside the internal antiforgery cookie. Attributes: `HttpOnly = false`, `Secure = true`, `SameSite = Lax`, `Path = "/"`, no `Domain`. Value is `antiforgery.GetAndStoreTokens(ctx).RequestToken`; SPA echoes it in the `X-XSRF-TOKEN` header. `__Host-` naming keeps posture parity with `__Host-RunCoach` and `__Host-Xsrf`.
2. **`[ApiController]` automatic 401/403** (R-061). ASP.NET Core 10 detects API endpoints in the cookie auth handler and returns JSON 401/403 instead of redirecting. No `OnRedirectToLogin` override needed.
3. **Antiforgery-attribute choice on MVC controllers.** *Superseded by DEC-055* — the original decision selected `[ValidateAntiForgeryToken]` per-action on `[ApiController]` classes; DEC-055 flips this to `[RequireAntiforgeryToken]` + `AddControllers()` + a bridge middleware after implementation surfaced that `[ValidateAntiForgeryToken]` requires `AddControllersWithViews()`.
4. **`options.User.RequireUniqueEmail = true`.** T02.1 omitted this (default `false`); Identity only enforced `RequireUniqueUserName`. Duplicate registrations surfaced as `DuplicateUserName` instead of `DuplicateEmail` — a latent bug if `UserName` ever decouples from `Email`. One-line fix in T02.4.

**Rationale:**

- `__Host-` naming parity across all three cookies simplifies audit — any deployment that weakens the prefix weakens all three at once.
- .NET 10's automatic API-endpoint 401/403 shipped GA November 2025; prior versions required an `OnRedirectToLogin` override.
- `RequireUniqueEmail` defaults-off is historical; every modern template sets it true.

**Alternatives considered (and rejected):**

- Angular-convention `XSRF-TOKEN` cookie name — breaks `__Host-` parity; RTK Query reads from any name.
- Keep an `OnRedirectToLogin` override — dead code under .NET 10's automatic behavior.

**Cross-references:**

- Microsoft Learn `security/anti-request-forgery` and `security/authentication/api-endpoint-auth`.
- DEC-050 (`__Host-` cookie precedent), DEC-052 (`IdentityErrorCodeMapper`), DEC-055 (supersedes bullet 3).
...
**Date:** 2026-04-22
**Status:** Final — supersedes DEC-054 bullet 3.
**Category:** Auth substrate / Slice 0 / T02 post-implementation correction
**Informed by:** R-062 (`batch-19e-antiforgery-attribute-net10.md`) + direct verification of `aspnetcore release/10.0` source.

**Decision:** State-changing MVC actions use `[RequireAntiforgeryToken]` from `Microsoft.AspNetCore.Antiforgery`, paired with `AddControllers()` (not `AddControllersWithViews()`) and a bridge middleware after `UseAntiforgery()` that converts `IAntiforgeryValidationFeature.IsValid == false` → `400` with a `ProblemDetails` body.
...
// ...
app.UseAntiforgery();
app.UseAntiforgeryProblemDetailsBridge();   // Infrastructure/AntiforgeryBridgeMiddleware.cs
app.MapControllers();
```
...
The bridge writes a `ProblemDetails` (`type = https://runcoach.app/problems/antiforgery-validation-failed`, title `"Antiforgery validation failed."`, detail pointing the SPA back at `/xsrf`) via `IProblemDetailsService.TryWriteAsync`, so the SPA's `ProblemDetails`-on-4xx base query needs no special case. `/xsrf` needs no carve-out — `GET` is already exempt via the middleware's `IsValidHttpMethodForForm` check.

**Rationale:**

- **The middleware does not short-circuit on validation failure.** `AntiforgeryMiddleware.InvokeAwaited` sets `IAntiforgeryValidationFeature(IsValid: false)` and unconditionally calls `_next`. Verified against `src/Antiforgery/src/AntiforgeryMiddleware.cs` in `release/10.0`. Minimal APIs bridge this automatically via form-binding; MVC does not.
- Without the bridge, MVC state-changing POSTs have two broken shapes: (a) request with a body → MVC's `FormValueProviderFactory` throws `InvalidOperationException` → `500`; (b) request without a body (e.g. `POST /logout`) → no exception, action runs as if validation passed. Both observed in T02.5's integration matrix before DEC-055 landed.
- `[RequireAntiforgeryToken]` (in `Microsoft.AspNetCore.Antiforgery`, per dotnet/aspnetcore#49237) emits `IAntiforgeryMetadata` — the metadata contract `UseAntiforgery()` reads. Works identically for MVC, Minimal APIs, and future Razor Pages without a view-engine dependency.
- `AddControllers()` drops the Razor view engine that `AddControllersWithViews()` would pull in solely to register the MVC antiforgery filter. Microsoft's own recommendation for API projects.
- `[IgnoreAntiforgeryToken]` doesn't implement `IAntiforgeryMetadata` — would be ignored by `UseAntiforgery()` anyway. Unused.
- R-062 errata: the artifact claimed the middleware "short-circuits with a 400." It does not — the bridge middleware is the MVC-side adapter the artifact's pattern implicitly relies on. The rest of R-062 holds.

**Retrospective:** T02.4 shipped `[ValidateAntiForgeryToken]` against parent-task guidance without surfacing the deviation; T02.5 then flipped to `AddControllersWithViews()` to fix the DI error, cementing the drift. Pattern for future: surface sub-task deviations from prescribed shape in the commit message or follow-up entry, not silently.

**Alternatives considered (and rejected):**

- Keep `[ValidateAntiForgeryToken]` + `AddControllersWithViews()` — works, but contradicts parent-task guidance, carries unused view engine, relies on dotnet/aspnetcore#22189 (open seam).
- Inline `app.Use` delegate instead of named class — the bridge grew past 3 lines (ProblemDetails emission), so a named class is clearer.
- Plain `400` with no body — forced the SPA into a special case; the ProblemDetails body makes the response consistent with every other 4xx.
- Controller-class `[RequireAntiforgeryToken]` — `/xsrf`, `/login`, and `/me` don't need it; per-action is clearer here. Reconsider for larger state-changing surfaces.
- MVC authorization filter instead of middleware — filter runs too late; form-binding already threw.

**Cross-references:**

- R-062 artifact: `docs/research/artifacts/batch-19e-antiforgery-attribute-net10.md` (with errata above).
- `aspnetcore release/10.0` `src/Antiforgery/src/AntiforgeryMiddleware.cs`.
- dotnet/aspnetcore#49237 (introduced `RequireAntiforgeryTokenAttribute`), dotnet/aspnetcore#22189 (antiforgery-without-views seam).
- DEC-050, DEC-054 bullet 3.
...
**Date:** 2026-04-22 (confirmed under two research passes — R-063 + R-064).
**Status:** Final.
**Category:** Developer environment / Slice 0 / T02.6 follow-up
**Informed by:** R-063 (`batch-20a-dotnet10-sdk-container-sigill-apple-silicon.md`) + R-064 (`batch-20b-sve2-sigill-second-opinion-verification.md`) + empirical verification on an Apple M4 Pro host.

**Decision:** `backend/Dockerfile` remains the CI image-build path for x86_64 runners but is **not** usable for the local dev loop on Apple Silicon (M3 / M4 / M5). `CONTRIBUTING.md` promotes Path B (host `dotnet run` + `docker compose up -d postgres redis`) as the primary inner loop on all hosts; Path A (`tilt up`) is labeled "x86_64 Linux only, until .NET 11." SDK + runtime base images are pinned by digest (SDK 10.0.203 / runtime 10.0.7; digests `sha256:8a90a47…3982fc` / `sha256:55e37c7…d9741d4`). No ENV JIT knobs ship — they were probed exhaustively and none cleared the fault deterministically on the reference hardware.

**Root cause:**

`dotnet/runtime#122608` (milestone 11.0.0, label `arm-sve`): .NET 10's CoreCLR emits non-streaming SVE2 opcodes whose Apple M3/M4/M5 silicon implements only inside SME streaming mode. Linux guests under Virtualization.framework synthesize `HWCAP2_SVE2` from `ID_AA64ZFR0_EL1` without filtering; `Sve2.IsSupported` returns `true`, the JIT emits the opcodes, the CPU raises UNDEF → `SIGILL` / exit 132. The flaw is in every shipped .NET 10 SDK since pre-GA PR #115117; no 10.0.x servicing backport is planned.

**Why no ENV workaround shipped:**

R-063's narrow-knob recommendation (`DOTNET_EnableArm64Sve=0` + `_Sve2=0`, including SME variants) and the master-switch (`DOTNET_EnableHWIntrinsic=0`) each tested 5/5 FAIL on the reference hardware; Rosetta / QEMU-driver / resource bumps also unchanged. R-064 confirmed the env-var names are correct verbatim from `jitconfigvalues.h` — the fault lives in `src/native/minipal/cpufeatures.c` detection probe (pre-CLRConfig), SME2 instructions with no .NET 10 knob surface (dotnet/runtime#121787 milestones SME to 11.0), or R2R-compiled BCL methods — none reachable from the env-var layer. R-064 rejected all 15 candidate workarounds with primary-source citations. Partial escapes (Docker Desktop ≥ 4.39; Colima QEMU + `cortex-a72`) are documented in CONTRIBUTING.md as opt-in hatches, not project posture.

**What shipped:**

- `backend/Dockerfile` — digest-pinned both stages; top-of-file comment captures the upstream bug, workaround log, and .NET 11 re-enablement trigger. A defensive `ENV` block disabling every SVE/SVE2 knob from `jitconfigvalues.h` is included — no-op on x86_64 CI, doesn't clear the arm64 fault, but reduces JIT-side emission surface if the image is ever accidentally built on arm64.
- `CONTRIBUTING.md` — Path B is the primary dev loop on all hosts; Path A documented as "x86_64 Linux only, until .NET 11," linking #122608 and this DEC.
- `docker-compose.yml` — unchanged; Apple-Silicon contributors use the `postgres redis` subset.

**Alternatives considered (and rejected):**

- Ship `DOTNET_EnableHWIntrinsic=0` as a hopeful mitigation — doesn't reliably clear the fault; 5–20% perf tax on x86_64 CI and healthy arm64 production (Graviton/Ampere).
- Delete `backend/Dockerfile` — still the CI image-build path; will be the Apple-Silicon dev-loop path again on .NET 11.
- Force Rosetta (`--platform=linux/amd64`) — fails separately on this Colima profile (SIGABRT 134); 1.5–3× CPU tax; hides real arm64 incompatibilities.
- Pin an older SDK digest — flaw shipped in pre-GA PR #115117; older digests also lose CVE-2026-40372 (DataProtection HMAC bypass) fixed in 10.0.203.
- `packages.lock.json` + `--locked-mode` + verify-container-restore harness (R-063 §6) — deferred as orthogonal workflow changes; Dockerfile comment + digest pins act as the manual-review gate for a solo-dev workflow.

**Re-enablement trigger:**

When Microsoft ships the first .NET 11 SDK, bump both digests and build `backend/Dockerfile` on an Apple M4/M5 host five times. If all pass, lift the "x86_64 Linux only" labeling. Re-check triggers (SDK servicing releases, #122608 comments/labels/PRs, Colima/Lima CPU-feature-mask support, Apple VZ feature-mask API, Docker Desktop LinuxKit mask docs) are enumerated in the R-064 artifact §D.

**Cross-references:**

- R-063: `docs/research/artifacts/batch-20a-dotnet10-sdk-container-sigill-apple-silicon.md`.
- R-064: `docs/research/artifacts/batch-20b-sve2-sigill-second-opinion-verification.md`.
- dotnet/runtime#122608 (upstream bug, milestone 11.0.0), #121787 (SME intrinsics, milestone 11.0), #112605 (closed .NET 9 precursor).
- containers/podman#28312 (closed "not planned").
- DEC-045, DEC-046, DEC-049.
...
**Why superseded:** the per-collection composition documented here did not, in practice, eliminate the Marten advisory-lock + `IDocumentStore` shutdown races on the shared assembly fixture — they recurred non-deterministically across worker dispatch rounds (#99, #110, #111). DEC-064 reverts to assembly-wide `DisableTestParallelization = true` as the canonical mode and codifies per-collection isolated databases as the future re-parallelization path when suite runtime forces it.

---

## DEC-062: `EfCoreSingleStreamProjection` registers via `opts.Add(...)`; document projections register via `opts.Projections.Add(...)` (Marten 8.32 EF projection registration shape)
...
- DEC-060 / R-069: `docs/decisions/decision-log.md` § DEC-060 / `docs/research/artifacts/batch-23a-marten-ef-dual-write-atomicity.md` — DEC-062 corrects the registration shape that DEC-060's implementation depends on.
...
## DEC-072: WorkoutLog metrics — open `jsonb` bag + single-sourced canonical keys (Slice 2b)

**Date:** 2026-05-31
**Category:** Backend / Persistence / EF Core / API contract
**Status:** Accepted — locked for the Slice 2b spec; first implemented in Slice 2b.
**Drives:** The `WorkoutLog` flexible-metrics store and the canonical metric-key contract shared by manual entry now and HealthKit/Strava/Garmin ingestion later.
**Cites:** R-077 (`docs/research/artifacts/batch-28a-efcore10-npgsql-heterogeneous-jsonb-metrics.md`); R-080 Part 2 (`docs/research/artifacts/batch-28d-meai-evalcache-prompt-binding-and-healthkit-keys.md` § Part 2).
**Builds on:** DEC-041 (unit value objects), DEC-060/DEC-062 (handlers emit events; EF projections own state), DEC-066 (OpenAPI→Zod codegen + drift gate), DEC-033 (HealthKit is the post-MVP-0 ingestion priority).

**Decision:**

- **Storage.** `WorkoutLog.Metrics` is a single nullable `string` property mapped to a PostgreSQL `jsonb` column via `.HasColumnType("jsonb")`; the API owns System.Text.Json (de)serialization at the boundary. Fallback when a typed CLR member is wanted: `Dictionary<string, JsonElement>` via a `ValueConverter` + a serialize-based `ValueComparer`. **Not** an owned record / `ComplexProperty().ToJson()` — EF Core 10 cannot map an open-ended dictionary to JSON (it is detected as a property bag and throws).
- **Wire shape.** Expose `Metrics` as an **open map** (`IDictionary<string, JsonElement>?` → OpenAPI `additionalProperties` → Orval `z.record(...)`), keeping the core fields a closed/strict object so the DEC-066 drift gate stays meaningful on them. Model `splits` as a **named `Split[]`** typed property (not a key inside the free map) so it is typed on the client.
- **Canonical keys, single-sourced.** One C# file `Modules/Training/Constants/WorkoutMetricKeys.cs` — `const string` set + a `WorkoutMetricKey` enum surfaced through a small catalog DTO so the enum flows to the frontend via codegen; manual entry and wearable auto-fill write identical keys. Canonical set + units (camelCase): `rpe` (int 1–10), `hrAvg`/`hrMax` (bpm, derived statistics on import), `calories` (kcal), `hrv` (ms SDNN — daily context, not a per-run number), `sleepScore`/`recoveryScore` (0–100), `cadence` (**full steps/min** — double Strava's single-leg API value), `elevationGain` (m), `power` (W), `weather` (object), `terrain` (string enum), `splits` (array). **Reserve names now, populate later** (zero-migration switch-on when a wearable source ships): `verticalOscillation` (cm), `groundContactTime` (ms), `strideLength` (m — convert Garmin cm/mm), optional `cadenceMax`. Pace is **always derived** and stored as `paceSecPerKm` — never trust a source's own pace field. Per-lap split object: `{ index, distanceMeters, durationSeconds, paceSecPerKm, averageHeartRate? }`.
- **Value objects.** `Distance`/`Duration` are core typed columns (not metrics), mapped via the repo's **first `ValueConverter`** (`Distance` ↔ `double` meters; `Duration` ↔ `long` ticks). A `readonly record struct` needs no custom `ValueComparer`. Unit-bearing keys inside the bag use canonical SI by key-name convention (e.g. `elevationGain` = meters), never `{value, unit}` pairs.
- **Indexing.** None at MVP-0 — ship the `jsonb` column un-indexed. A whole-column GIN index is modelable (`.HasIndex(w => w.Metrics).HasMethod("gin")`) but expression indexes on extracted keys are not (`HasIndex` rejects non-simple expressions); add `CREATE INDEX … (((metrics->>'rpe')::numeric))` via `migrationBuilder.Sql(...)` when the first value-query slice lands and the table grows (~10k rows/cohort). `WorkoutLog` implements `Marten.Metadata.ITenanted` for parity with `UserProfile` (mandatory if it later becomes an `EfCoreSingleStreamProjection` target under conjoined tenancy).

**Rationale:** the bag must accept unknown keys with no DDL per metric, round-trip through codegen so the frontend never hand-maintains the key list (the contract-drift class DEC-066 exists to kill), and leave value-query/index reachability open without re-architecting — a single opaque `jsonb` scalar satisfies all three, and any later index is a pure additive migration. Canonical SI-by-key-name keeps the map flat and queryable. The canonical-key enum-via-catalog is the only mechanism that keeps the map open while still single-sourcing the keys through codegen. Choosing keys/units that already match HealthKit/Strava/Garmin source names means future auto-fill populates the same bag with no migration.

**Verified (external claims independently re-checked 2026-05-31, workflow `verify-slice2b-research-claims`):** Npgsql `string`/`JsonDocument`→`jsonb` with developer-owned STJ — confirmed (Npgsql JSON-mapping docs). GIN modelable / expression-index not — confirmed (Npgsql Indexes docs; npgsql/efcore.pg #2568 open). Swashbuckle `Dictionary<string,T>`→`additionalProperties` and Orval→`z.record` (not `strictObject`, unaffected by `override.zod.strict`) — confirmed. `RequireNonNullablePropertiesSchemaFilter` promotes only the **outer** object's `required` and does not recurse into a dictionary sub-schema, so the open map is **not** wrongly forced strict — confirmed in repo source. `OwnsOne().ToJson()` closed-record precedent + no existing `ValueConverter` + `Distance`/`Pace` `readonly record struct` — confirmed in repo. HealthKit/Strava key names + units — confirmed, **except** the humidity metadata key is `HKMetadataKeyWeatherHumidity` (not `HKMetadataKeyHumidity`). **Correction to the artifact:** efcore #26903 (open-dict→JSON) is OPEN **milestoned 11.0.0** (not Backlog); substance unchanged — no GA path in EF Core 10.

**Open verification (impl-time):** assert the generated `swagger.json` leaves the `Metrics` sub-schema as an open `additionalProperties` map (not `additionalProperties:false`); smoke-test the generated `*.zod.ts`; round-trip test (sparse bag + splits, absent ≠ null) + a `dotnet ef migrations add` no-op probe proving a new canonical key needs no DDL.

**Alternatives considered:** owned dictionary via `ToJson()` — rejected, does not work (efcore #26903). Typed owned record, one property per metric — rejected, DDL per new metric, defeats open-endedness. GIN index now — rejected, write overhead at MVP-0 and it doesn't serve the future range queries anyway (those need expression indexes).

---

## DEC-073: Synchronous in-handler LLM-failure policy — SDK-only retry, typed exception contract, `Kind=Error` envelope (Slice 2b spec; first live in Slice 3)

**Date:** 2026-05-31
**Category:** Backend / LLM / Resilience / API contract
**Status:** Accepted — decided now in the Slice 2b spec; first exercised live in Slice 3 (no live LLM call sits on a 2b request path).
**Drives:** The user-facing behavior and retry semantics when an in-handler LLM call fails on a synchronous `InvokeForTenantAsync` endpoint (onboarding, plan-regeneration, future adaptation/conversation).
**Cites:** R-078 (`docs/research/artifacts/batch-28b-wolverine6-transient-llm-failure-error-policy.md`).
**Builds on:** DEC-057 (single-handler/session/transaction), DEC-058 (Pattern-B flat-slot+discriminator structured output), DEC-059 (sanitizer), DEC-060 (Marten-backed `IIdempotencyStore`), DEC-066 (codegen), DEC-052 (Anthropic first-party SDK).

**Decision:**

- **Single retry layer = the Anthropic SDK, and only there.** Configure the SDK for interactivity: `MaxRetries = 2` (its default; 3 attempts total), per-attempt `Timeout ≈ 30s` (down from the 10-min default), total wall-clock bounded by the inbound request `CancellationToken`. No Wolverine `RetryWithCooldown` on the command, no Microsoft.Extensions.AI resilience `DelegatingChatClient`, no `Microsoft.Extensions.Http.Resilience` on the LLM path — exactly one retry layer, no double-retry.
- **`ICoachingLlm` exception contract.** It throws only `TransientCoachingLlmException` / `PermanentCoachingLlmException`; the translation from Anthropic SDK exception types lives in `ClaudeCoachingLlm` (the lowest layer that still understands Anthropic types). Transient = exhausted 429/5xx/529/408/connection/IO; terminal = 400/401/403/404/422/malformed-data.
- **Wire envelope.** Synchronous LLM-backed endpoints return a `Kind = Error` branch on the response DTO (flat nullable slots `errorMessage`, `retryable`, `retryAfterSeconds`) with **HTTP 200** — Pattern-B safe (no `oneOf`/`anyOf`), Orval/Zod-friendly (DEC-066), uniform across endpoints. The handler maps both domain exceptions to this branch before they reach `ErrorHandlingMiddleware` (which stays the catch-all for truly unexpected exceptions).
- **Observability.** Terminal failures set the M.E.AI `UseOpenTelemetry()` GenAI span status to ERROR with `error.type`, and increment a non-paging `coaching.llm.failures` counter tagged `error.type`/`outcome` (`transient_exhausted` | `terminal`). Dashboard only — no alert route.
- **Idempotency on retry.** The `IIdempotencyStore` marker is written in the same Marten transaction as the side effects; a failed attempt rolls the marker back so the same `idempotencyKey` is safely reusable on the client's "try again," and a successful turn is never re-invoked on replay.
- **Frontend.** Because the failure is HTTP 200 `data`, the RTK Query `retry()` baseQuery wrapper is **not** used here; the component branches on `kind`: `Error & retryable` → manual "Try again" (re-dispatch with the same `idempotencyKey`; disable for `retryAfterSeconds` if present); `Error & !retryable` → terminal state, no retry. No auto-retry for MVP-0.

**Rationale:** the SDK already classifies transient statuses correctly and honors server-specified delay, so a second layer only multiplies latency and token spend on a multi-call chain. An LLM failure on an interactive coaching turn is a *domain outcome*, not a transport fault — returning it in-band keeps the typed success channel and the single codegen schema uniform and lets the client branch rather than catch. Co-transactional idempotency gives ACID rollback for free (cleaner than Stripe's "cache errors" model). Decided now so the contract is uniform across every LLM endpoint, even though 2b itself has no live LLM call on a request path.

**Verified (independently re-checked 2026-05-31):** Wolverine applies only `Retry`/`RetryWithCooldown` to inline `InvokeAsync`/`InvokeForTenantAsync` — `MoveToErrorQueue`/dead-lettering is durable-pipeline only (so `Program.cs`'s three `MoveToErrorQueue` policies are inert for the synchronous LLM path) — confirmed (Wolverine docs). SDK auto-retries 2× on connection/408/409/429/5xx, 10-min default timeout, `MaxRetries`/`Timeout` configurable — confirmed (platform.claude.com C# SDK docs + `ClientOptions.cs`). M.E.AI `DelegatingChatClient` forwards exceptions unwrapped — confirmed (still gated by the impl-time integration test below). `IIdempotencyStore` is Marten-document-backed — confirmed in repo. **Corrections folded in:** (1) **C9 refuted** — the official SDK *does* expose `ClientOptions.HttpClient` (public settable; the docs page merely omits it), so `Http.Resilience` *is* attachable; the SDK-only-retry choice therefore stands as a deliberate single-layer decision, not a "no seam exists" necessity. If external resilience is ever layered, set the injected client `Timeout = InfiniteTimeSpan` and set SDK `MaxRetries = 0` to avoid double-retry. (2) **C7 taxonomy** — actual hierarchy is `AnthropicException → AnthropicServiceException → AnthropicApiException → { Anthropic4xxException (incl. `AnthropicRateLimitException` for 429, `AnthropicBadRequest`/`Unauthorized`/`Forbidden`/`NotFound`/`UnprocessableEntity`), Anthropic5xxException }`, plus `AnthropicIOException`/`AnthropicSseException`/`AnthropicInvalidDataException`/`AnthropicUnexpectedStatusCodeException`; **there is no `RetryAfter` accessor** — read `retry-after` from raw response headers via `WithRawResponse`. The catch-order in the artifact still works (specific `AnthropicRateLimitException` before the `AnthropicApiException` catch-all).

**Open verification (impl-time, Slice 3):** one integration test forcing a 429 and a 400 through the full chain (`ICoachingLlm → ClaudeCoachingLlm → AnthropicStructuredOutputClient → AsIChatClient → SDK`) to confirm exceptions arrive **unwrapped** at `ClaudeCoachingLlm`; confirm the `retry-after`-via-raw-headers read on 12.24.0; re-verify on any `Anthropic` SDK bump (beta — breaking changes possible in minor/patch).

**Alternatives considered:** RFC 9457 `ProblemDetails` 503 — rejected, breaks the uniform discriminated-response contract and makes RTK treat the turn as a thrown error. Bare HTTP 200 partial-success — rejected, no machine `retryable` signal. Wolverine `RetryWithCooldown` on the command — rejected (would double-retry; and inline invokes don't dead-letter anyway). `Http.Resilience` layer — rejected for the LLM path (would double-retry), though now known to be attachable.

---

## DEC-074: Eval-cache ↔ prompt-template binding — SHA-256 sentinel manifest with local pre-commit detection (Slice 2b)

**Date:** 2026-05-31
**Category:** Testing / LLM eval / Quality pipeline
**Status:** Accepted — locked for the Slice 2b spec; resolves the Slice 1 carry-forward "eval-cache invalidation on prompt-template edit."
**Drives:** Surfacing a stale committed eval cache **locally at commit time** instead of as a post-merge CI Replay failure.
**Cites:** R-080 Part 1 (`docs/research/artifacts/batch-28d-meai-evalcache-prompt-binding-and-healthkit-keys.md` § Part 1).
**Builds on:** DEC-039 (committed eval-cache CI replay), DEC-043 (quality pipeline), the existing `rerecord-eval-cache.sh` / `prepush-eval-check.sh` / `check-eval-cache-ttl.sh` lefthook hooks.

**Decision:** Adopt a committed **SHA-256 sentinel manifest** of `Prompts/*.yaml` (`Prompts/.prompt-hashes.sha256`), checked primarily by a **glob-scoped lefthook `pre-commit` hook** (`check-prompt-hashes.sh`, fires only when a staged file matches `Prompts/*.yaml`) and regenerated by `rerecord-eval-cache.sh` immediately after it patches the cache TTL — so cache and manifest always move together. A small **`EvalTestBase` static-ctor hash assertion** is the CI / `--no-verify` backstop (same ~10 lines of hashing logic, identical remediation message: "run `rerecord-eval-cache.sh`"). Do **not** adopt prompt-version-in-cache-key as the detector (it busts the key but detects nothing earlier than a plain edit already does, and depends on a human remembering to bump). Normalize the hash tool across platforms (`shasum -a 256` on macOS vs `sha256sum` on Linux).

**Rationale:** prompt text already travels inside `messages[]`, so a prompt edit already busts the M.E.AI cache key → a clean Replay MISS → CI failure; correctness is not at risk, only *timing*. The sentinel is near-instant, needs no test build, and encodes exactly the invariant that matters ("the committed cache was last recorded against these prompt contents"). The pre-commit hook catches it earliest and costs nothing on non-prompt commits; the fixture backstop covers `--no-verify` and CI (where git hooks don't run).

**Verified (independently re-checked 2026-05-31):** M.E.AI 10.6.0 `CachingChatClient.GetCacheKey` JSON-serializes messages + options + `additionalValues`; `ChatOptions.AdditionalProperties` is **not** `[JsonIgnore]` (whereas `Tools`/`RawRepresentationFactory` are) so it participates in the key — the batch-17c `runcoach.prompt_version` trick works, and `DistributedCachingChatClient.CacheKeyAdditionalValues` is the cleaner first-class knob — confirmed (MS Learn 10.6 reference). `ConversationId` set ⇒ caching disabled (`EnableCaching`) — confirmed; eval requests must leave `ConversationId` null. Scripts + lefthook wiring + CI `EVAL_CACHE_MODE=Replay` + `YamlPromptStore` computes no content hash — confirmed in repo.

**Open verification (impl-time):** match the exact `sha256sum` byte layout (lowercase hex, two spaces, bare filename, `\n`) between the script and the C# backstop, or normalize both sides. **Caveat to document:** the manifest does **not** cover M.E.AI **package-version** cache-key drift (the docs warn the key is not stable across releases) — pair any `Microsoft.Extensions.AI*` bump with a `rerecord-eval-cache.sh` run.

**Alternatives considered:** version-into-cache-key + runtime check — rejected, doesn't detect earlier than an edit already does and depends on manual version bumps. Pre-commit hook that re-runs the affected evals in Replay — rejected, heaviest option (compiles + runs `dotnet test` on every prompt-touching commit; duplicates `prepush-eval-check.sh`).

---

## DEC-075: Workout-logging form & history conventions — RHF + Zod-v4 optional-numeric coercion, generated-schema derivation, sparse-metric rendering (Slice 2b)

**Date:** 2026-05-31
**Category:** Frontend / Forms / Design system
**Status:** Accepted — locked for the Slice 2b frontend spec; reusable across later logging/adaptation surfaces.
**Drives:** The log form and history list, and the project-wide conventions every later multi-field/optional-metric form inherits.
**Cites:** R-079 (`docs/research/artifacts/batch-28c-react19-progressive-disclosure-logging-form-zod-v4.md`).
**Builds on:** DEC-063 (reduced-motion contract), DEC-066 (generated Zod), DEC-068 (frontend error patterns), DEC-070 (design tokens + contrast gate).

**Decision:**

- **Form stack.** Stay on React Hook Form + `zodResolver` + shadcn `Form` (not React 19 Actions — no client validation, no server action under RTK Query). Use one `useForm<Input, any, Output>` (three generics, because coercion makes input ≠ output) with `shouldUnregister: false`; wrap the optional-metrics section in shadcn `Collapsible` with fields **always mounted** (never `{open && …}`) so a filled-then-collapsed metric still submits. Default the section **closed** on a fresh log; auto-open if any optional field is pre-filled.
- **Zod-v4 optional-numeric coercion (the load-bearing footgun).** Optional numeric fields resolve empty → `undefined`, never `NaN`/`0`. Use a per-field `setValueAs: v => v === '' || v == null ? undefined : Number(v)` (or a schema-level `z.preprocess(blank→undefined, z.number().optional())`). **Never** `z.coerce.number()` on optional fields (`Number('') === 0` → blank validates as 0). Do not use `.default()` on optional metrics. "Blank" = `.optional()`; `.nullish()` only if the backend distinguishes null.
- **Derive the form schema from generated code.** Build it from the Orval-generated request-body schema via `.pick().extend()` so the metric set can't drift from the backend contract. Keep the generated body schema **refinement-free** (in Zod 4.4.3 `.pick()/.omit()/.partial()` throw on refined object schemas), and layer frontend-only refinements with `.extend()`. The **response/history** schema uses `z.looseObject(...)` so future server-added metric keys survive parse; the renderer ignores keys it has no label for.
- **Inputs & history.** Numerics use `type="text" inputMode="decimal"` (avoid `type=number` spinbutton footguns); duration is a single decimal "minutes" field for MVP-0. History renders a `<dl>` showing **only present** metrics, labels from a single shared `metric-meta` key→label/unit map (so `hrAvg → "Avg HR"` lives in one place). **Splits are display-only** in MVP-0 (no `useFieldArray`) — a one-line summary + a nested `Collapsible` `<table>` (lazy-rendered).
- **Data flow.** Pessimistic RTK Query create (await success → invalidate `History`/`LIST` → refetch), consistent with the DEC-073 idempotency/"try again" contract; optimistic insert deferred until a measured latency problem (and then with invalidate-on-error, not `patchResult.undo()`).
- **Design-system compliance.** Semantic tokens only; new tokens must pass the `check-contrast` gate in Latte + Mocha; every `transition-*`/`animate-*` paired with a `motion-reduce:` variant (DEC-063); add `role="alert"` to the shared `FormMessage` for assertive error announcement (it currently lacks it) — coordinate with task #560.

**Rationale:** the log form is the product's highest-friction surface; wrong optional-numeric coercion blocks a minimum-payload save, wrong `shouldUnregister` silently drops collapsed metrics, and a hand-maintained metric list reintroduces the contract-drift class DEC-066 kills. These are reusable conventions, hence a DEC rather than spec-local notes.

**Verified (independently re-checked 2026-05-31, incl. empirically against the repo's installed zod 4.4.3 + RHF 7.76 + @hookform/resolvers 5.2.2):** `z.coerce.number()` coerces `'' → 0`; RHF `valueAsNumber` → `NaN` on empty — confirmed (colinhacks/zod #2461; RHF docs). `shouldUnregister` defaults false and retains unmounted values — confirmed (RHF useForm docs). Zod v4 `z.coerce.*` input type = `unknown`; `.default()` short-circuit/output-type change; error-API unification — confirmed. `FormMessage` lacks `role="alert"`; `Collapsible`/`RadioGroup`/`Textarea`/`Form` primitives present; `numeric-turn-input` uses `Controller`+`valueAsNumber`; generated Zod barrel present — confirmed in repo. **Correction folded in:** `.safeOmit()`/`.safePick()`/`.safePartial()` do **not** exist in Zod v4 (only `.safeExtend()`), and `.extend()` does **not** throw on refined schemas — so the rule is "derive sub-schemas before refining (or rebuild via `z.object(schema.shape)`); never reference `.safeOmit`."

**Open verification (impl-time):** the minimum payload (all metrics blank) must `safeParse` to success with every metric `undefined`, and an explicit `0` must survive as `0`; verify Collapsible mount/`hidden` behavior on the installed `radix-ui` 1.4.3 (`shouldUnregister:false` is the load-bearing guarantee regardless).

**Alternatives considered:** React 19 Actions — rejected (no client validation; no server action under RTK Query). `z.coerce.number().optional()` — rejected (the `''→0` footgun). Optimistic create now — deferred (a once-per-run action; pessimistic is simpler and safer with the idempotency/retry contract). Pill/badge grid or table for sparse metrics — rejected in favor of `<dl>` (correct key/value semantics; tables imply row/column relationships a flat metric set doesn't have — reserved for the genuinely tabular splits).
...
## DEC-085: Slice 4B conversation core — user-scoped conversation + confirm-then-commit logging + classify-then-route; streaming posture and the `IncompleteCoachingLlmException` contract (Slice 4B)

**Date:** 2026-06-24
**Category:** Conversation architecture / LLM streaming / structured output / persistence
**Status:** Accepted — records the load-bearing product + contract decisions locked in the Slice 4B brainstorm (design `docs/plans/mvp-0-cycle/slice-4b-conversation-core.md`) and spec (`docs/specs/19-spec-slice-4b-conversation-core/`, gitignored/working-tree-only), plus the one contract-shaping choice taken during PR1 (the errored-turn exception). The spec is the implementation authority; this entry pins the decisions that set product identity and the public `ICoachingLlm` contract so they survive across sessions.
**Drives:** the interactive streaming coaching conversation — its persistence model, its intent/logging behavior, the streaming adapter's failure surface, and the eval approach.
**Builds on:** DEC-079 (Slice 3 read-only conversation substrate this extends), DEC-060 (handler emits events; one Marten transaction per handler), DEC-073 (synchronous LLM-failure typed-exception contract — extended to streaming here), DEC-058 (Pattern-B structured output — first non-adaptation classifier), DEC-074 (prompt-hash sentinel), DEC-084 (gruff-direct voice — streamed output inherits; no runtime scrubber), R-082/R-083/R-084.

**Decision:**

- **D1 — User-scoped conversation, plan-scoped adaptation.** The interactive conversation persists across plan regenerations via a net-new `UserId`-keyed `Conversation` event stream + projection; the shipped plan-scoped `ConversationLogView` (proactive adaptation/safety turns) is left untouched. RunCoach is "my coach", not "a coach for this plan."
- **D2 — Q&A + conversational logging.** The conversation both answers grounded questions and logs workouts conversationally; an intent classifier is therefore required, not optional.
- **D3 — Classify-then-route via a structured pre-call.** A fast Pattern-B Haiku classify triages each message into `{Question | WorkoutLog | Ambiguous}` and extracts a `StructuredLogDraft` for logs. (The classify targets Haiku via a per-call model override; it cannot pin `Temperature 0` — see the PR3a correction below — so its determinism comes from constrained decoding, not sampling.) Rejected: tool-use / function-calling; an explicit `/log` affordance. On classifier failure → `Kind=Error`; never guess intent.
- **D4 — Conversational logging is confirm-then-commit.** A parsed log is advisory until an explicit button-driven Confirm; only then does it flow through the unchanged Slice 2b create + Slice 3 adaptation path. This is the determinism boundary: no LLM judgment mutates the plan without a human confirm.
- **Streaming-posture corrections** (verified against `backend/src` during spec-writing; these correct assumptions the design doc inherited from R-082/R-083):
  - **No runtime trademark/voice scrubber on a live stream.** Trademark scrubbing runs on the complete buffered reply (scrub-on-persist); voice is enforced by the prompt + eval guard (DEC-084, no runtime scrubber). Live-token trademark exposure is an accepted low-risk residual.
  - **Two-transaction persistence.** The co-transactional `IIdempotencyStore` + single-Wolverine-`SaveChanges` model cannot span a multi-second stream; the user turn and the assistant turn are two separate idempotent writes (client GUID + server-derived GUID).
  - **Eval = buffer-then-assert over the non-streaming path.** R-083's premise (`ResponseCachingChatClient` / `CoalesceStreamingUpdates`) does not exist in this repo's eval harness; the streamed answer is recorded/replayed through `GetResponseAsync` and asserted over the buffered text. (Research re-trigger candidate if real streaming-replay is later wanted.)
  - **A Haiku production binding is a net-new capability** (Haiku is eval-judge-only today) — the classifier adds it via a per-call model-override parameter on `GenerateStructuredAsync`. (`Temperature` was assumed a sibling capability here, but is not viable — see the PR3a correction below.)
- **The errored-turn contract = `IncompleteCoachingLlmException` (PR1 design lock).** `ICoachingLlm.StreamAsync` (net-new, `IAsyncEnumerable<string>` text deltas) signals a stream that ended without a usable, complete reply — `max_tokens` truncation, context-window overflow, or refusal, which R-084 confirms the SDK reports as a clean enumeration end (not an exception) — by throwing a net-new `IncompleteCoachingLlmException : CoachingLlmException` carrying `bool Retryable` + `IncompleteReason {MaxTokens, ContextWindowExceeded, Refusal}`. This extends the DEC-073 hierarchy by one type so the totality contract stays uniform (every terminal/error condition throws: Transient/Permanent for call failures, Incomplete for unusable content, `OperationCanceledException` for client abort). It carries the content-disposition axis ("discard the partial, persist an errored marker"), which is orthogonal to Transient/Permanent's retry-the-call axis.

**PR3a correction (2026-06-26) — no temperature / sampling control on current models.** The Anthropic SDK 12.31.0 marks `MessageCreateParams.Temperature` / `TopP` / `TopK` `[Obsolete]`, and the API rejects any non-default value with HTTP 400 on current Claude models (Sonnet 4.6 / Haiku 4.5); there is no `seed`. The "classifier at `Temperature 0`" requirement (D3, Unit 4, and the Unit 7 eval) is therefore **not implementable**. PR3a adds only the per-call model-override on `GenerateStructuredAsync` (no temperature parameter); classifier determinism rests on **constrained decoding** (the byte-stable frozen schema) + a deterministic prompt. The Unit 7 eval's "deterministic at `Temperature 0`" gate becomes a "stable under constrained decoding" gate. Verified empirically by a reflection probe and the `CS0618` build error.

**PR3b note (2026-06-27, #231) — the classifier never converts units.** Unit 4's first cut had the classifier prompt convert runner units to SI (miles/km→meters, minutes→seconds), which crosses the deterministic-computation-layer rule (LLMs never do pace/zone/distance/time math — that belongs in unit-tested code). The shipped design instead has the classifier extract the runner's actuals *in the units they stated* — `StructuredLogDraft` carries a distance value + `RunnerDistanceUnit {Kilometers,Miles,Meters}` and the duration as whole h/m/s components — and the deterministic, unit-tested `WorkoutDraftUnitConverter` does the SI conversion server-side when `StructuredLogDraftMapper` builds the create DTO at confirm time (1609.344 m/mi exact). Because constrained decoding cannot express numerical bounds, `MessageIntentOutputValidator` also gained a post-deserialization range check (non-positive/non-finite distance, undefined unit, negative or >59 duration components, zero total → reject), and a reject coerces to `Ambiguous` (the existing bias-to-ask path) so an impossible draft never reaches the confirm/create path. `Anthropic:ClassifierModelId` now fails fast at startup (alongside `ApiKey`/`ModelId`) rather than silently falling back to the coaching model.

**PR4 note (2026-06-27, #233) — SSE endpoint wired; the post-heartbeat error contract and a test-harness carryover.** PR4 shipped `POST /api/v1/conversation/messages` as designed (durable-first user turn → deterministic safety gate → classify → route to `token` / `safety` / `card` / `clarification` / `error` / `done` SSE frames over `StreamAsync`, antiforgery-protected, buffering off, flush-per-frame, client-abort-is-not-a-500). The deep-review + CodeRabbit cycle refined two contract points worth pinning: (1) the controller's post-heartbeat cancellation catch is narrowed to `OperationCanceledException` / `IOException` (gated on `RequestAborted`) so a genuine bug thrown during a cancelled request still surfaces to the logging catch instead of being swallowed as a clean disconnect; and (2) the workout-log confirmation card emits **no** `done` frame and persists no turn id — it commits nothing until the PR5 Confirm — which the `DoneFrame` doc now states. One carryover is logged in the cycle plan § Captured During Cycle: the SSE integration helper `SeedActivePlanAsync` seeds `RunnerOnboardingProfile` with an explicit `TenantId` that the `ITenanted` EF tenant filter then hides from the authenticated request, so the on-plan card-prescription path can't be asserted end-to-end under test yet — a test-seam gap, not a product bug; the pace-band-mapping risk is guarded by the new `CandidatePrescriptionDtoTests` unit test in the meantime.

**PR5 note (2026-06-29, #235) — confirm-then-commit endpoint shipped; ack persisted after the adaptation, three deep-review LOW carryovers closed in-PR.** PR5 shipped `POST /api/v1/conversation/logs/confirm` as designed: `ConfirmConversationalLogService` maps the confirmed `StructuredLogDraft` onto the unchanged Slice 2b `CreateWorkoutLogRequestDto` (via `StructuredLogDraftMapper`, EF-row idempotency key derived from the card's `ClientMessageId` per DEC-077), commits through the unchanged `WorkoutLogService.CreateAsync`, runs the identical `IAdaptationEvaluationDispatcher` post-create seam, and only then persists a short gruff-direct LLM ack as a coach turn (idempotent on the server-derived coach turn id) so the ack can never preempt or contradict an Amber referral (DEC-081 posture). A terminal/lost-race adaptation (`Kind=Error`) returns the scripted "saved; review retrying" ack rather than re-calling the LLM; an out-of-range confirmed draft surfaces an `ArgumentException` from value-object construction that the controller maps to a 400 (nothing committed), the same boundary `WorkoutLogsController.CreateLog` enforces. The CodeRabbit + deep-review cycle resolved all threads; three deep-review LOW findings were fixed in-PR (`3eb2471`): (1) `PersistAckAsync` now splits ack composition and persistence into two try/catch scopes with distinct `[LoggerMessage]` methods, so a non-LLM compose fault is no longer mislabeled a persistence failure; (2) a defensive `Create(IEvent<CoachMessagePosted>)` overload on `InteractiveConversationProjection` seeds `ConversationView.UserId` from the event for the unreachable-today coach-first stream (kept for the client-agnostic future iOS consumer / projection rebuild); (3) the `ArgumentException`→400 branch gained an integration test (negative-distance draft → `BadRequest` + `application/problem+json` + zero committed logs + zero ack generations). The PR4 SSE-seed carryover (the `SeedActivePlanAsync` `ITenanted` tenant-filter gap) persists — PR5 does not touch the SSE plan-grounding surface, so it remains for PR6/PR7 to align with the timeline test's seeding.

**PR6 note (2026-06-30, #237) — frontend streaming UX shipped; a cold-start reconcile fix and the orphaned read-path carryover.** PR6 shipped the React streaming conversation surface: `useCoachStream` (the hand-rolled SSE `fetch` reader over `POST /api/v1/conversation/messages` — token/safety/card/error/done frames, lazy antiforgery seed, `401 → loggedOut`, abort on re-send/unmount, retry with a fresh client message id), the `CoachChat` panel (timeline + live bubbles + retry + safety UI) now mounted on `home.page.tsx` in place of the Slice 3 read-only `ConversationPanel`, the `LogConfirmationCard`, the confirm-then-commit wiring through `confirmConversationalLog`, and the Edit→`/log` pre-fill via `draftToWorkoutLogFormFields`. The CodeRabbit + deep-review cycle (all threads resolved) fixed a cold-start race — the optimistic timeline reconcile via `updateQueryData` silently no-ops when the mount-time timeline GET is still in flight (verified against the installed RTK 2.12.0; `upsertQueryData` would be clobbered by the resolving GET), so `appendTimelineTurn` now falls back to `invalidateTags(['Conversation'])` when no patch was produced (RTK's delayed invalidation refetches server truth) — added the card-draft shape guard in `parseCard`, and the abort-lifecycle + non-401/null-body/network error-path hook tests. Two carryovers are logged in the cycle plan § Captured During Cycle: the `ConversationPanel`/`getConversationTurns` read chain is now orphaned (comment reworded, dead-code removal deferred), and the PR4 SSE-seed test-harness gap remains for PR7 (PR6 is frontend-only). PR7 (streaming-conversation eval + classifier accuracy + DEC-074 manifest) is the last 4B PR.

**Rationale:** D1/D2 set product identity (a persistent coaching relationship that can log conversationally), so they warrant a decision-log entry rather than living only in the gitignored spec. The streaming-posture corrections matter because the design doc inherited R-082/R-083 assumptions the code-surface mapping disproved; recording them prevents a future session from re-introducing a runtime scrubber, a single-transaction persistence design, or a streaming-replay eval the harness cannot support. The `IncompleteCoachingLlmException` choice keeps the streaming failure surface a single uniform throw-based contract; modeling truncation as a yielded value (while errors throw) or folding it into `Permanent` (which would say "the call is unretryable" rather than "this turn is incomplete") were both weaker — see the `StreamAsync` XML docs and the spec § U1.

**Alternatives considered:** (1) keep the plan-scoped `ConversationLogView` and re-key it to `UserId` — rejected (an event-sourcing migration of a shipped projection that entangles deterministic safety/adaptation turns with free-form chat). (2) Q&A-only (no conversational logging) — rejected by the builder; the conversational-logging magic is the point. (3) For the errored turn: reuse Transient/Permanent — rejected (conflates retry-the-call with discard-the-turn); yield an update-union — rejected (an in-band sentinel R-084 warns against; splits the terminal model). (4) Leave all of this on the spec + cycle-plan lock without a DEC — rejected: D1/D2 set product identity and the errored-turn extends the DEC-073 public contract, both warranting a durable entry.

**Cross-references:** Slice 4B design `docs/plans/mvp-0-cycle/slice-4b-conversation-core.md`; spec + pr-strategy `docs/specs/19-spec-slice-4b-conversation-core/` (gitignored, working-tree-only); DEC-079 / DEC-060 / DEC-073 / DEC-058 / DEC-074 / DEC-084; R-082 (`batch-30a`), R-083 (`batch-30b`), R-084 (`batch-30c`).

---

## DEC-086: Slice 4C — hybrid form-first onboarding + km/miles as a frontend-display-only preference in a dedicated settings store; amends DEC-041's imperial-display phasing (Slice 4C)

**Date:** 2026-07-01
**Category:** Onboarding UX / unit system / settings persistence
**Status:** Accepted — records the load-bearing decisions locked in the Slice 4C brainstorm (design `docs/plans/mvp-0-cycle/slice-4c-onboarding-units.md`). 4C is decomposed into **4C-units** (unblocked, ships first) and **4C-onboarding** (research-gated on R-085, ships second) plus two cleanup tasks; the per-piece spec is written fresh at build time. This entry pins the decisions that set product shape and cross-cutting constraints so they survive across sessions.
**Drives:** the onboarding intake redesign, the km/miles display affordance, and where the unit preference lives.
**Builds on:** DEC-047 (event-sourced onboarding stream + dual projection — extended, not replaced), DEC-060 (handler emits events; projections own state — a deterministic form still originates events), DEC-058 (Pattern-B frozen onboarding schema — any field change edits it + regens the DEC-074 manifest), DEC-084 (gruff-direct onboarding voice guard — any prompt rewrite re-records the eval), DEC-041 (canonical SI storage + convert-at-display; imperial display was deferred to MVP-1 — **amended here**), DEC-010 / root `REVIEW.md` (LLM never converts units), DEC-085 PR3b (`WorkoutDraftUnitConverter` deterministic km/miles precedent), DEC-075/DEC-063 (form + animation conventions).

**Decision:**

- **D1 — Decomposition.** 4C-units (units foundation, no LLM/onboarding coupling) ships first and unblocked; 4C-onboarding (form-first hybrid intake) is research-gated on R-085 and consumes the units foundation; two standalone cleanup tasks ride along.
- **D2 — Onboarding intake = hybrid form-first.** A structured form is the primary intake and **originates `AnswerCaptured` events deterministically** (form submission produces the same event the LLM extraction produces today), so the DEC-047/DEC-060 projections (`OnboardingView` + `UserProfile`) and the deterministic completion gate survive unchanged — only the event's *origin* changes. The LLM shrinks to optional per-area free-text nuance (extracted verbatim per DEC-085; never converts units). This dissolves the WeeklySchedule slot-merge loop (structured slots collected directly, no multi-turn free-text assembly) and the resume-path input-kind mismatch (the form carries structured state). Rejected: full deterministic form with no LLM (loses nuance); keep conversational + fix the loop via field-level partial-slot merge or transcript-feedback (non-obvious event-model change; transcript-feedback breaks the byte-stable Pattern-B prompt cache).
- **D3 — Units = frontend display-only conversion; prompt stays km-native.** Conversion lives on the frontend at display (and miles→km on write for numeric inputs); the wire and plan-gen prompt stay km/SI, and the LLM never converts (DEC-010/DEC-041). A shared unit-aware formatter reads the persisted preference. Pace is net-new work — an *inverse* conversion (`sec/km × 1.609344 = sec/mi`), not the forward distance constant, and `WorkoutDraftUnitConverter` covers distance + duration only — with a formula-derived display spec (mm:ss/mi rounded to whole seconds; race distances stay literal proper nouns; track intervals stay meters). Rejected: backend API-boundary conversion with explicit `{value, unit}` DTO fields (DEC-041 MVP-1; versioned-API + codegen churn); SI-unify the km-native plan layer first (touches the plan structured-output contract; MVP-1).
- **D4 — Canonical preference = a dedicated `UserSettings` store, decoupled from onboarding.** A mutable EF row keyed by `userId` (last-write-wins, `GET`/`PUT`), decoupled from the onboarding aggregate so units are editable post-onboarding without mutating the onboarding event stream. The enum stays the shipped `PreferredUnits {Kilometers, Miles}` (does **not** resurrect DEC-041's never-built `UnitPreference {Metric, Imperial}` — YAGNI). A Settings-page Units toggle writes via `PUT`; the onboarding `PreferencesAnswer.PreferredUnits` slot becomes vestigial for units. Rejected: a purpose-built settings-edit event on the onboarding stream (keeps units entangled with onboarding answers); reuse the whole-record `POST /answers/revise` (client resends all four `PreferencesAnswer` fields, mutates the onboarding aggregate for a settings edit); defer editability entirely.
- **D5 — Cleanup.** Delete the orphaned Slice-3 read chain (`getConversationTurns` + the two generated hooks + `useConversationTurns` + `ConversationPanel` + their specs — no Slice-3-style "Explain-the-change" surface will be re-mounted; `CoachChat` is the surface now). Fix the `SeedActivePlanAsync` `ITenanted` test-harness gap (drop the explicit `TenantId` to match the timeline test's seeding; restore the on-plan card-prescription assertion). Both non-blocking, standalone.
- **D6 — Known MVP-0 limitation.** Frontend-display-only converts structured numbers but leaves LLM free-text prose km-native (the coach says "8 km" while the card shows "5.0 mi"). Making the coach speak miles requires pre-converting prompt data (DEC-041 MVP-1; breaks the byte-stable prompt cache). Accepted for MVP-0 — the input-side "speak either unit, I'll convert" affordance is honored (deterministic parse per DEC-085); output prose is the deferred half.

**Amends DEC-041:** DEC-041 phased the imperial DISPLAY formatter and `UnitsNet` to MVP-1 and named a `UnitPreference {Metric, Imperial}` enum + formatting interface that were never built. 4C pulls a frontend-display-only imperial affordance into MVP-0 (not the API-boundary conversion, not the LLM-speaks-miles half, not locale auto-detect, not `UnitsNet` — those remain MVP-1) and reconciles the naming by keeping the shipped `PreferredUnits {Kilometers, Miles}` as canonical rather than introducing the unbuilt `UnitPreference`.

**Rationale:** D1/D2 set onboarding's product shape (a light, form-first intake that dissolves a correctness bug while preserving the event-sourced substrate), so they warrant a durable entry rather than living only in a per-piece spec. D3/D4 pin the cross-cutting units architecture (display-only, dedicated store) that a future session could otherwise re-open in the wrong direction (a runtime prompt conversion or an onboarding-aggregate settings edit). Recording the DEC-041 amendment prevents "imperial is MVP-1" from being read as blocking 4C.

**Alternatives considered:** (1) conversational onboarding + server-side loop fix — rejected (event-model/cache risk); (2) backend API-boundary or SI-unify units — rejected (blast radius vs a self/family MVP-0 display toggle); (3) preference on the onboarding aggregate — rejected (couples settings to onboarding answers); (4) resurrect `UnitPreference {Metric, Imperial}` — rejected (YAGNI, the shipped enum is already codegen'd); (5) LLM speaks miles now — rejected (DEC-041 MVP-1, breaks prompt cache). See the design doc for the full brainstorm trail.

**Cross-references:** Slice 4C design `docs/plans/mvp-0-cycle/slice-4c-onboarding-units.md`; R-085 research prompt `docs/research/prompts/batch-31a-form-first-onboarding-redesign.md`; R-085 artifact `docs/research/artifacts/batch-31a-form-first-onboarding-redesign.md`; DEC-047 / DEC-058 / DEC-060 / DEC-041 / DEC-010 / DEC-084 / DEC-085 / DEC-074 / DEC-075 / DEC-063; cycle-plan § Captured During Cycle (the three live-pass rows + two 4B carryovers this triages).

**Research integration (R-085, 2026-07-04):** the form-first artifact landed and was integrated into the design doc (§ "R-085 findings integrated"); every load-bearing claim was verified against `backend/src`/`frontend/src`. It resolves D2's "how" without reopening the locked shape, and refines one point: **onboarding needs no LLM call at all** — per-area nuance is stored verbatim text rendered into *later* coaching prompts, not an onboarding-time extraction (so D2's "optional LLM free-text nuance … extracted verbatim per DEC-085" narrows to "optional free-text stored on existing slot fields"; DEC-085's verbatim-extract pattern belongs to the log-write path, not onboarding). Resolved specifics: (1) event origination = a `SubmitStructuredAnswers` `[AggregateHandler]` over `FetchForWriting<OnboardingView>` appending the **existing** `AnswerCaptured` event, whole-record-per-topic, no new event type — the codebase's existing command idiom; (2) **`Description`-field audit is load-bearing and not uniform** — `PrimaryGoal`/`CurrentFitness`/`WeeklySchedule`/`Preferences` carry `Description`, `InjuryHistory` carries `PastInjurySummary`, but **`TargetEventAnswer` has no free-text field**, so a nuance box there is the one path that edits `OnboardingSchema.Frozen` and busts the DEC-074 manifest (default: omit the TargetEvent nuance box); (3) both `slice-4-conversation.md` carry-forwards (server-driven `SuggestedInputType`; the state-machine contract) are **obsoleted** by form-first; (4) retire the conversational path via an `onboarding.formFirst` feature-flag cutover — in-flight streams complete via the form with zero migration because both emit `AnswerCaptured` — then hard-remove `onboarding-v1`/`OnboardingTurnHandler`/`POST /turns` and archive the onboarding voice eval; (5) retire the multi-slot free-text-merge eval scenarios (the form structurally removes the failure mode); (6) day-of-week = radix `ToggleGroup`, no `react-day-picker` — no new dependency. The 4C-onboarding research gate is now clear; the spec is unblocked.

---

## DEC-087: MVP-0 close F-LIVE-1 — bounded corrective-hint retry on macro-plan validation rejection; amends the DEC-073/DEC-080 "no re-prompt" posture (MVP-0 close-out)

**Date:** 2026-07-07
**Category:** Plan generation / LLM retry policy
**Status:** Accepted — pins the F-LIVE-1 fix from the 2026-07-07 MVP-0 close live-pass (design `docs/plans/mvp-0-cycle/mvp-0-close-live-pass-fixes.md`). The per-piece spec is written fresh at build time; this entry records the decisions that change a documented posture and slightly broaden the written disposition so they survive across sessions.
**Drives:** how `PlanGenerationService.GeneratePlanAsync` handles a deterministic macro-validation rejection.
**Builds on:** DEC-073 (Anthropic adapter exception taxonomy + SDK-owned transient retry; "a retry is the caller's decision, MVP-0 does not auto-retry" — **amended here** for the macro-validation case), DEC-080 (single-call terminal-reject-with-nothing-staged posture — **amended here**), DEC-082 / Slice 3B F3 (`MacroPlanOutputValidator`, `PlanHorizon`, `PlanGenerationRejectedException`), DEC-057 (six-call macro/meso/micro chain).

**Decision:**

- **D1 — Bounded retry, macro tier only, inside the service.** On a `MacroPlanOutputValidator` rejection, `GeneratePlanAsync` re-invokes the macro tier up to `CoachingLlmSettings.MacroValidationMaxRetries` extra times before throwing. The retry lives inside the service so both callers (onboarding terminal branch, regenerate handler) benefit; the macro tier is the first LLM call, so a re-roll costs one call and no meso/micro work is wasted. No backoff delay — this is a bad-output re-roll, not a rate-limit (the SDK owns transient/429 backoff below the adapter per DEC-073). Rejected: retry at each call site (duplicates policy across two handlers); retry the whole six-call chain (wastes meso/micro on a macro-only defect).
- **D2 — Corrective-hint re-prompt, not blind re-roll.** Each retry appends a deterministic correction suffix to the macro *user* message naming the exact defect (`PhaseSumMismatch`: observed phase-week sum vs. declared `TotalWeeks`; `HorizonMismatch`: required target total weeks vs. emitted). This converts an open-ended "produce a globally self-consistent plan" task into a narrow arithmetic reconciliation the model reliably fixes, and is essential for `HorizonMismatch` (a systematic constraint-misread a blind resample may reproduce). Cache-safe by construction: the suffix rides the never-cached user message; the `Ephemeral1h`-marked system block is untouched, so the retry reuses the primed cache exactly as meso/micro already do. The attempt-0 macro message is byte-identical to today's (no suffix), preserving the input-prompt-stability contract and eval-cache replay. Rejected: blind re-roll (relies on decoder nondeterminism; weak/uninformed for `HorizonMismatch`); reinforcing the base prompt on attempt 0 (edits byte-stable base bytes → forces an eval re-record — tracked as a separate follow-up).
- **D3 — Scope = any invalid macro (`PhaseSumMismatch` AND `HorizonMismatch`).** Both are stochastic LLM-output errors the corrective hint addresses; retrying one but not the other has no principled basis. This **broadens the written F-LIVE-1 disposition**, which named only `PhaseSumMismatch`. Recorded explicitly so the broadening is auditable. Rejected: `PhaseSumMismatch`-only (matches the literal disposition but leaves the sibling stochastic rejection unmitigated).
- **D4 — Config-bound budget, default 1 retry.** `CoachingLlmSettings.MacroValidationMaxRetries` (default `1` → up to 2 macro attempts; `N` means `N+1` attempts, mirroring the SDK `MaxRetries` field's own semantics). Live evidence showed the 2nd attempt succeeded and the corrective hint makes a single retry highly effective; a config knob lets production raise it without a redeploy while capping worst-case latency before the clean fallback failure (~2× macro latency at the default). Rejected: hardcoded constant (no production tuning); unbounded/loop-until-valid (chases a stochastic decoder).
- **D5 — Terminal behavior unchanged on exhaustion.** After the budget is spent, throw `PlanGenerationRejectedException(lastViolation)` exactly as today — onboarding's existing handled 422 and the atomic no-staged-events rollback are preserved. The retry only shrinks the probability of reaching the terminal state; it never changes what happens there. Doc comments on `PlanGenerationRejectedException` and the service's macro-validation block are updated from "no re-prompt" to "bounded re-prompt then terminal reject", referencing this DEC.
- **D6 — Observability.** Each retry is warn-logged (attempt number + violation); the macro-attempt count is stamped on the plan-generation chain span and the `runcoach.plan.generation.completed` metric (success and failure tag bags) so dashboards distinguish "recovered on retry" from "first-try clean" and can watch for a rising base failure rate the retry would otherwise mask.

**Rationale:** D1/D2/D5 change a *documented* decision (DEC-073's "MVP-0 does not auto-retry" and DEC-080's single-call terminal-reject), so they warrant a durable entry rather than living only in a per-piece spec — a future session could otherwise re-read those DECs as still-binding and re-remove the retry. D3 broadens the written disposition, which must be auditable. D4/D6 pin the tunable and the visibility that keep the retry from silently masking a regression.

**Alternatives considered:** (1) blind re-roll — rejected (uninformed, weak for `HorizonMismatch`); (2) reinforce the base prompt instead of / in addition to retrying — deferred (eval-re-record blast radius; complementary, not a substitute for a recovery mechanism); (3) `PhaseSumMismatch`-only scope — rejected (leaves `HorizonMismatch` unmitigated for no principled reason); (4) hardcoded retry count — rejected (no production tuning); (5) move plan-gen fully async/decoupled from onboarding submit so a rejection doesn't block the request — deferred (larger architecture change; the bounded synchronous retry closes the finding for MVP-0). See the design doc for the full trail.

**Cross-references:** design `docs/plans/mvp-0-cycle/mvp-0-close-live-pass-fixes.md` (F-LIVE-1); DEC-073 / DEC-080 / DEC-082 / DEC-057; cycle-plan § Captured During Cycle (2026-07-07 MVP-0 close-out row + Open follow-ups digest); memory `project_mvp0_close_live_pass`. Sibling finding F-LIVE-2 (meso↔micro consistency) is a separate follow-up with its own forthcoming DEC.

---

## DEC-088: MVP-0 close F-LIVE-2 — deterministic meso↔micro consistency validator + bounded corrective-hint micro retry (MVP-0 close-out)

**Date:** 2026-07-07
**Category:** Plan generation / cross-layer consistency
**Status:** Accepted — pins the F-LIVE-2 fix from the 2026-07-07 MVP-0 close live-pass (design `docs/plans/mvp-0-cycle/mvp-0-close-live-pass-fixes.md`). The per-piece spec is written fresh at build time; this entry records the decisions that establish a new cross-layer gate and choose its rejection policy so they survive across sessions.
**Drives:** how `PlanGenerationService.GeneratePlanAsync` handles a week-1 micro layer that disagrees with the meso week-1 template.
**Builds on:** DEC-087 (bounded corrective-hint macro retry — the mechanism mirrored here at the micro tier), DEC-080 / DEC-083 (`RestructureConsistencyCheck` — the deterministic cross-layer arithmetic-gate precedent, but that gate is terminal-only; this one retries), DEC-057 (six-call macro/meso/micro chain), DEC-073 (adapter exception taxonomy; the terminal reject is an *expected* rejection, not a transport fault).

**Finding.** The meso and micro layers are produced by independent structured-output calls: the meso week-1 template names which days are `Run`/`Rest`/`CrossTrain` and each run day's `WorkoutType`, and the micro call independently lays out the week-1 workouts. `BuildMicroUserMessage` recaps only three meso scalars (phase, weekly target km, deload flag), never the per-day slots, so live Sonnet can and does emit a micro week that disagrees with the meso week on run-day count or workout type. The home page renders the meso week narrative beside micro-derived workout cards, so the contradiction is user-visible. No cross-layer validator existed.

**Decision:**

- **D1 — Deterministic run-day validator, micro vs. meso week 1.** New pure static `MesoMicroConsistencyValidator.Validate(weekOneMeso, micro)` (in `Modules/Coaching/Models/Structured/`, beside `MacroPlanOutputValidator`) reconciles the micro workouts against the meso week-1 run slots. The comparison is **exact** over run days: the set of `(dayOfWeek → WorkoutType)` pairs from the meso `Run` slots must equal the set from the micro workouts, catching both the run-day-count mismatch and the swapped-workout-type symptoms the finding named. Returns a pure discriminator (`MesoMicroConsistencyResult { IsValid, Violation }` with `RunDayCountMismatch` / `RunDaySetMismatch` / `WorkoutTypeMismatch`); the concrete mismatch is recomputed by the service's correction builder, mirroring `MacroPlanOutputValidator`'s pure shape. Rejected: an aggregate-only count check (misses the type swap the finding explicitly called out); a distance-sum check like F4 (the layers disagree on structure, not km).
- **D2 — Meso is the source of truth; retry the micro tier only.** On rejection the service re-invokes **only** the micro tier — the last of the six calls — with a corrective hint. The four meso calls and the macro call have already run and are treated as authoritative (macro → meso → micro is a refinement hierarchy; the micro layer *expands* the meso week). Re-rolling meso week 1 is far more invasive (meso weeks 2-4 already ran independently and `mesoEvents[0]` is already staged), so it is rejected. A re-roll therefore costs exactly one micro call.
- **D3 — Bounded corrective-hint retry, not terminal-reject-only and not deterministic reconciliation.** The retry mirrors DEC-087: a config-bound `CoachingLlmSettings.MicroValidationMaxRetries` (default `1` → 2 attempts; `N` → `N+1`; clamped against a compiled ceiling of 5), no backoff, each retry appending a deterministic suffix to the micro user message naming the exact run-day schedule to reproduce (day + type per meso run slot) and the schedule the rejected micro produced. Rejected: **deterministic reconciliation** (patch micro to match meso without re-prompting) — infeasible, because adding a missing run day requires synthesizing its full distance/pace/segment detail, which only the LLM can produce; **terminal-reject-only** (F4/DEC-083 style) — would re-introduce exactly the stochastic-failure dead-end DEC-087 had just removed at the macro tier, for the same class of cross-item invariant the model violates only occasionally.
- **D4 — Cross-train and rest days are out of run-day scope.** The comparison covers run days only: a meso `CrossTrain`/`Rest` slot is not expected in the micro run set, and a micro `WorkoutType.CrossTrain` entry is excluded from the run-workout set — mirroring the `CrossTrain` exclusion in `RestructureConsistencyCheck`. This keeps the gate from false-rejecting on cross-train placement while still enforcing the run schedule the finding is about. A meso run slot with a null `WorkoutType` constrains presence only (a run workout must exist that day; its type is unconstrained). A duplicate micro workout on one day is reported (the run-workout count exceeds the meso run-day count).
- **D5 — Terminal behavior: a sibling exception, same 422.** On budget exhaustion the service throws a new `MesoMicroConsistencyRejectedException` (sibling to the macro `PlanGenerationRejectedException`), and the onboarding completion controller catches it into the **identical** re-submittable 422 with nothing staged (the atomic-rollback posture is unchanged). Rejected: generalizing `PlanGenerationRejectedException` to carry either violation — it would perturb the macro exception's public surface and the merged F-LIVE-1 tests for no behavioral gain; two focused sibling exceptions read more honestly (macro-arithmetic vs. cross-layer-consistency) and the controller maps both to one shared 422 helper.
- **D6 — Observability.** A new `MicroAttempts` tag is stamped on the chain span and the `runcoach.plan.generation.completed` metric (success and failure tag bags), `total_calls` now includes micro retries, and each retry/rejection is warn-logged — mirroring the DEC-087 D6 macro-attempt visibility so a dashboard can watch how often the retry fires and whether it masks a rising base inconsistency rate.
- **D7 — Cache-safe, zero eval re-record.** Attempt 0 (`correction=null`) is byte-identical to the pre-DEC-088 micro message, and the suffix rides the never-cached user message (the `Ephemeral1h` system-block cache is untouched), exactly as DEC-087. The plan-generation eval suite does not route through `PlanGenerationService` at all (it calls a test-local structured-output helper against the legacy `AssembleAsync` path), so no eval fixture cache key changes and no DEC-074 manifest regen is triggered — empirically consistent with PR #271's zero eval-cache diff.

**Rationale:** D2/D3/D5 are the load-bearing choices — they establish that a stochastic cross-layer inconsistency is recovered by re-rolling the cheap downstream tier against the authoritative upstream one, rather than terminally dead-ending the runner (the DEC-087 lesson, applied one tier down) or attempting an impossible deterministic patch. D1/D4 pin the comparison semantics so the gate catches the finding's exact symptoms without false-rejecting on cross-train placement. D6/D7 keep the retry visible and cache-neutral.

**Alternatives considered:** (1) terminal reject with no retry — rejected (re-introduces the dead-end); (2) deterministic reconciliation — rejected (cannot synthesize missing workout detail); (3) re-roll meso — rejected (invasive, upstream already staged); (4) reinforce the micro base prompt with the meso day-slots on attempt 0 to lower the base inconsistency rate — **deferred** as a complementary root-cause follow-up (it changes attempt-0 live behavior; the retry mechanism closes the finding on its own, mirroring DEC-087's deferral of the unconditional phase-sum prompt reinforcement); (5) generalize the macro rejection exception — rejected (perturbs a merged, tested surface for no gain). See the design doc for the full trail.

**Cross-references:** design `docs/plans/mvp-0-cycle/mvp-0-close-live-pass-fixes.md` (F-LIVE-2); DEC-087 / DEC-080 / DEC-083 / DEC-057 / DEC-073; cycle-plan § Captured During Cycle (2026-07-07 MVP-0 close-out row + Open follow-ups digest); memory `project_mvp0_close_live_pass`. Sibling finding F-LIVE-1 (macro-arithmetic retry) is DEC-087.

---

## DEC-089: SPLIT / Alpine UI redesign — design-language adoption, tab-bar IA, dark-default, and the cycle's contract decisions (SPLIT UI cycle)

**Date:** 2026-07-07
**Category:** Frontend design system / IA / API contract scope
**Status:** Accepted — locks the decisions that let the SPLIT / Alpine redesign cycle (`docs/plans/split-ui-cycle/cycle-plan.md`) be built across fresh sessions. Design source of truth is the builder-produced handoff + design sheet committed at `docs/design/split-alpine/` (HANDOFF.md + split-alpine.dc.html); per-slice specs are written fresh at build time.
**Drives:** the entire visual layer (tokens, typography, IA, seven screen recompositions), plus four narrow wire additions and one prompt-assembly addition.
**Builds on:** DEC-070 (two-tier token architecture — kept, re-skinned), DEC-063 (Tailwind-only motion — kept), DEC-086 (form-first onboarding + units — the surfaces being restyled), DEC-085/DEC-077/DEC-081 (conversation/idempotency/safety contracts — preserved verbatim), DEC-082 (server week math the client now mirrors for display).

**Decision:**

- **D1 — Adopt SPLIT / Alpine wholesale as the design language.** Alpine primitive ramp (dark + light per handoff § 2) replaces Catppuccin inside the unchanged two-tier architecture; **dark becomes the `:root` default and light the `.light` override** (inverts the DEC-070-era polarity); Barlow Condensed / Barlow / IBM Plex Mono self-hosted (R-086 integrated 2026-07-07 with errata: static per-weight `@fontsource/*` at pins 5.2.8/5.2.8/5.2.7 + `fontaine@0.8.0` metric fallbacks — the artifact's `@fontsource-variable/*` primary recommendation does not exist on npm); the § 2/§ 3 geometry + typography role tables are law ("numbers condensed, labels mono"). The 2026-07-07 gap analysis classified all 58 handoff requirements; 28 are pure restyles because the semantic-token layer survives.
- **D2 — SPLIT renders now (user-ratified 2026-07-07).** The wordmark + brand strings ship while the trademark/domain search runs in parallel; the name is contained in the `Wordmark` component + a small string set so a forced rename is a bounded swap. Fallbacks recorded in the handoff (THRESHOLD / CADENCE / VERST).
- **D3 — The home digest's adaptation summary is composed client-side, deterministically, from the already-typed `PlanAdaptationDiff` payload.** No summary field is added to the event or timeline wire, and clamped LLM prose is not used as a headline. Rejected: a server-side summary field (new event/wire surface for a display concern; the typed diff already carries the facts) and prose-clamping (violates the design's never-parse/never-trust-prose contract).
- **D4 — The plan projection gains structured target-event fields** (name / distance / date) so THE BLOCK's goal chip is plan-scoped truth. Rejected: sourcing from `GET /onboarding/state` (reflects last onboarding answers, not the active plan; drifts on regenerate).
- **D5 — Two logging wire additions, narrowly scoped.** (a) A `GET` prescribed-slot-for-date endpoint reusing `PlanCalendar.ResolveSlot` + the existing prescription resolver (today reachable only via the SSE path) for the /log prescribed banner. (b) `IsOnPlan` + a nullable prescribed-title on the history-row DTO — a deliberate, display-scoped amendment of the MVP-0 stance that withheld the prescription snapshot from history rows; the snapshot itself stays private. Every wire change rides the full OpenAPI codegen chain.
- **D6 — Partial-parse log-draft states are deferred.** The backend never emits a partial `StructuredLogDraft` (all fields required; the classifier falls back to `Ambiguous`), so the designed `MISSING` affordance is unreachable UI. The reachable states (saving / success-receipt / failure) ship; partial extraction is a separate product decision, captured in the cycle plan.
- **D7 — Onboarding gains exactly one narrative free-text field** (`00 — IN YOUR OWN WORDS`), plumbed through the answers DTO → canonical record → projection → `AnswerCaptured` (additive) and injected verbatim into plan-generation prompt assembly beside the existing per-topic nuance fields — same unsanitized posture (accepted MVP-0 caveat, pre-public-release hardening item stands). This is the cycle's only LLM-touching change: DEC-074 manifest regen + targeted plan-gen fixture re-record, isolated in its own PR. THE RACE section gets no ADD DETAIL collapsible (no nuance field exists; none is added). **Build note (2026-09-05, Slice 5 spec):** the narrative reaches the prompt through C# composition in `ContextAssembler.BuildPlanGenerationUserMessage`, not a `Prompts/*.yaml` edit, so the DEC-074 manifest is unchanged (`check-prompt-hashes.sh --write` is a verified no-op) and the eval consequence narrows to recording ONE new fixture (`plan.dated-event-narrative.macro`); an empty narrative adds no prompt bytes, so every existing fixture replays byte-identical. The additive `AnswerCaptured.Narrative` field follows the DEC-091 / Slice 2 additive-nullable precedent (no upcaster); `backend/REVIEW.md` now records that exception.
- **D8 — Small closures:** the Settings footer version is a build-time constant from the frontend package version (no meta endpoint); Slice 6's first real SIGN OUT affordance must call `resetApiState()` on logout (pulls the known cross-account cache-leak follow-up — declined PR #174, previously deferred to pre-public release — forward because the affordance becomes reachable); the closing funded-key live pass doubles as the outstanding MVP-0 F-LIVE-1/F-LIVE-2 done-gate verification (user-ratified 2026-07-07: redesign first). *Slice 6 build note (2026-09-06, PR #365):* the version constant reuses `import.meta.env.VITE_APP_VERSION`, defined in `vite.config.ts` from `package.json` (bumped to 0.9.0) with an environment override, so the Settings footer, error telemetry and the OpenTelemetry service version share one source; sign-out runs `resetApiState()` after the logout POST and again in the cross-tab receiver.

**D5(b) title-source note (ratified 2026-07-16, shipped Slice 4 PR-A).** The prescribed-title field is the frozen `WorkoutType` enum name (nullable string `PrescribedWorkoutType`), rendered client-side via the existing `WORKOUT_TYPE_LABELS` map — not the LLM-authored `WorkoutOutput.Title`. `WorkoutPrescriptionSnapshot` carries no title at all, and the descriptive title lives only on `WorkoutOutput.Title`, which `RestructureDiffCalculator` diffs on adaptation; live re-resolution would let a later plan restructure retroactively rewrite an already-logged row's title. Zero migration, and symmetric with the SSE card's `CandidatePrescriptionDto`, which also carries only `WorkoutType`.

**Rationale:** D1/D2 commit the visual direction the builder designed end-to-end, on the cheapest structural path (the token architecture and every behavioral contract survive). D3–D6 keep the backend surface almost frozen for what is a visual cycle — each wire addition exists only where the client genuinely cannot derive the design's data (prescription resolution, on-plan truth, plan-scoped goal), and each rejected alternative either duplicated server logic client-side or widened the wire for a display concern. D7 is the one intake capability the design adds, isolated because it is the only change with eval-cache blast radius.

**Alternatives considered:** rebuilding on a component-library re-skin without the token swap (rejected — the two-tier layer is the cheap path); a server-side adaptation summary (rejected, D3); onboarding-state as goal-chip source (rejected, D4); client-side on-plan recomputation (rejected — requires exposing the full prescription snapshot, a wider surface than D5b); building the partial-parse UI as dead states (rejected, D6); deferring the redesign until after the standalone MVP-0 done-gate session (rejected by the user — redesign first, gate rides the closing pass).

**Cross-references:** cycle plan + eight slice docs under `docs/plans/split-ui-cycle/`; design assets `docs/design/split-alpine/`; R-086 (fonts, queued); DEC-070/063/086/085/082/077/081; the declined-PR-#174 cache-reset follow-up (pulled forward into Slice 6).

---

## DEC-090: Rolling plan horizon — sweeper-driven weekly extension, deterministic training-history graft, no LLM call in a user-facing request (Rolling Plan Horizon)

**Date:** 2026-07-13
**Category:** Plan generation / background processing / event contract
**Status:** Accepted — locks the design for `docs/plans/plan-horizon/rolling-horizon-plan.md`, itself synthesized from a three-design, three-adversarial-lens design brief. Specs are written fresh per-PR at build time.
**Drives:** how a plan's meso and micro tiers get populated past their current hard ceiling; a new `Modules/Training/Plan/Horizon/` module plus additive edits to `PlanProjection.cs`, `Infrastructure/MartenConfiguration.cs`, and `PlanGenerationService.cs`/`IPlanGenerationService.cs`.
**Builds on:** DEC-087/DEC-088 (bounded corrective-hint retry pattern, mirrored here for the new `MesoWeekOutputValidator`); DEC-073 (adapter exception taxonomy — the rejected per-log design's fatal flaw was narrowing this taxonomy incorrectly); DEC-074 (eval-cache↔prompt binding — this design is engineered to never trigger it); DEC-057 (macro/meso/micro chain, extended here).

**Problem.** Plan generation is structurally capped and never extends. `PlanEventSequence` validates the canonical event shape `[PlanGenerated, MesoCycleCreated×4, FirstMicroCycleCreated]` via a hard equality guard (`Mesos.Count == ExpectedMesoCount`, `ExpectedMesoCount = 4`, `PlanEventSequence.cs:33,69`), and `PlanGenerationService.MesoWeekCount` is a hardcoded `internal const int = 4` driving the generation loop (`PlanGenerationService.cs:136,143,341-343`). Adaptation only edits existing weeks (`PlanProjection.Apply(PlanAdaptedFromLog, ...)` does a `FindIndex`/`TryGetValue` in-place replace and explicitly skips, never synthesizes, a week absent from the projection) — nothing in the codebase ever appends meso week 5+. The micro tier is worse and already live-broken: `FirstMicroCycleCreated` is singular and hardcoded to week 1 (`PlanProjection.cs:142-149`), so **only week 1 of any plan ever has real prescribed workouts** — weeks 2-4 have meso day-slots (Run/Rest/CrossTrain) but nothing to log against, a day-8 bug in every plan today, not a week-5 edge case. The frontend's `resolveCurrentWeek` (`use-plan.hooks.ts`) has been masking this by clamping the displayed week to the last populated week (permanently 4); that clamp is correct and stays, but until this ships it has nothing but stale week-4 data to clamp against.

**Decision:**

- **D1 — Trigger: a timer-only `BackgroundService`, never a user request.** New `PlanHorizonSweeper : BackgroundService`, mirroring `IdempotencySweeper`'s tick shape (immediate run on boot, `await SweepAsync(ct)` then `await Task.Delay(interval, ...)`, no overlapping ticks). **Hard invariant: no LLM call this design adds ever runs inside a user-facing request** — onboarding completion, `POST /plan/regenerate`, workout-log create, and `GET /plan/current` are all byte-for-byte unmodified in request-time behavior and latency.
- **D2 — Cadence and buffer: 30-minute sweep interval, 2-week populated buffer.** Meso and micro stay populated 2 weeks ahead of the runner's date-derived current week, re-evaluated fresh every tick so an idle plan self-corrects rather than drifting.
- **D3 — Selection: lowest incomplete week, not `currentWeek + 1`.** A pure `SelectNextIncompleteWeek` picks the lowest week missing meso or micro, capped at `min(currentWeek + BufferWeeks, macro.TotalWeeks)`. This is also what backfills the live micro-tier bug for free: every plan live today has `MesoWeeks = {1,2,3,4}` but `MicroWorkoutsByWeek = {1}`, so the lowest incomplete week for all of them is week 2 — the sweeper fills it as the ordinary first thing it does on its first tick, the same code path as steady-state extension, not special migration machinery.
- **D4 — Contract: one new event, one new `Apply` overload, `PlanEventSequence` untouched.** New `MicroCycleCreated(int WeekIndex, MicroWorkoutListOutput Micro)`, structurally mirroring `MesoCycleCreated`; `FirstMicroCycleCreated` is not deleted or repurposed (Marten streams are append-only). `ExpectedMesoCount == 4` stays scoped exactly to the two existing `StartStream` call sites (onboarding, regenerate); extension never constructs a `PlanEventSequence`, it appends directly to the plan's existing stream.
- **D5 — Idempotency and concurrency: projection state is the source of truth; a marker is a disposable fast-path only.** A durable pre-check against the live projection (cheap, no expiry) is what actually prevents duplicate work; an `IIdempotencyStore` marker is a fast-path only, not authoritative, because `IdempotencySweeper` deletes any marker older than 48h regardless of purpose and this question gets re-asked every 30 minutes across a plan's multi-month life. `ExtendPlanHorizonHandler` gets its own chain-scoped `OnException<EventStreamUnexpectedMaxEventIdException>().RetryTimes(3).Then.MoveToErrorQueue()`, mirroring `EvaluateAdaptationHandler`, because the two handlers race on the same stream. Per-plan try/catch isolation inside each sweep tick; a `PlanHorizonExtensionState` circuit breaker trips after 5 consecutive failures per plan.
- **D6 — The `TrainingHistorySummary` graft: deterministic, non-LLM, suffix-only.** A new `TrainingHistorySummarizer` computes `WeeksCompleted`, `AdherenceRatePercent`, `CurrentSignalState` (reused `AdaptationSignalState`), `RestructureCount`, and `MostRecentWeeklyTargetKm` (the already-adapted ground truth, not macro's day-1 guess), threaded into the horizon-extension prompt via new sibling builder methods that never edit `BuildMesoUserMessage`/`BuildMicroUserMessage` and never touch the shared cacheable prefix. Per root `CLAUDE.md`'s deterministic-computation/LLM-coaching split, this arithmetic must never be LLM-derived.
- **D7 — Failure handling: no synchronous fallback, no new user-facing error surface.** Extension failure can only ever leave a plan "behind schedule," never broken — `PlanEventSequence`'s untouched all-or-nothing guard means a stream is either fully absent or already carries 4 complete meso weeks. The frontend clamp absorbs staleness; the circuit breaker gets a structured log line and an OTel metric only, no alerting built.
- **D8 — `MesoWeekOutputValidator` scope: required on the new extension path only, not backported.** Required here because an unvalidated `WeekNumber` mismatch hard-aborts the extension transaction (`PlanProjection.cs:111-115`). Backporting it to `GeneratePlanAsync`'s original 4-week bootstrap loop — which today has zero meso validation — is filed as a fast-follow, not part of this ship.

**Rejected alternatives:**
- **Generate all weeks upfront (Eager Full-Horizon).** Commits up to 20 weeks of exact workout prescriptions on day 1 with zero training-history signal — the "static plan generator... that never adjusts" anti-pattern `vision-and-principles.md` opens by rejecting, at 5x today's scale.
- **Generate on workout-log (JIT per-log dispatch off `WorkoutLogsController.CreateLog`).** Its copied `AdaptationEvaluationDispatcher` catch clause narrows only two Marten exceptions; with no surrounding try/catch in the controller and the meso/micro validator optional rather than required, a horizon-extension LLM failure would 500 the entire log-create request and discard an already-committed log write. It also targets only `currentWeek + 1`, which cannot backfill a multi-week gap or recover from one missed logging window.
- **1-week buffer.** A single delayed tick could let the runner catch up to an ungenerated week. **4-week buffer.** Generates weeks a mid-plan regenerate would discard before they're ever reached, and widens the blind-future-week gap the D6 graft narrows.
- **Ship the base sweeper without the D6 graft.** Not fatal, but it implements "guess now, correct later" rather than reasoning from real history at generation time, which the product's stated thesis calls for.
- **Port `EvaluateAdaptationCommand`'s marker-only idempotency verbatim.** That pattern only needs to answer "did we process this once, near creation" — this question is re-asked for months, which the 48h retention window can't support alone.

**Consequences:** Onboarding/regenerate latency and call count are unchanged — this design adds nothing to any user-facing request. Eval-cache re-record blast radius is zero: new sibling prompt-builder methods, existing `BuildMesoUserMessage`/`BuildMicroUserMessage` and all 5 existing `plan.*.mesoweek`/`micro` fixtures untouched, no `.prompt-hashes.sha256` touch, no DEC-074 trigger. `resolveCurrentWeek`'s clamp stays exactly as written — a correct defensive fallback for staleness — but its ceiling finally grows past 4, so in steady state it converges on the runner's real current week instead of permanently stalling. There is no backfill-migration problem and no legacy-data compatibility work: every plan currently in the dev database is throwaway test data, and every plan (dev or eventually production) self-heals via the ordinary D3 selection rule on its first sweep tick — the sweeper filling a missing week is the same code path as normal extension, not special machinery. NoAnchor (general-fitness, no race date) plan rollover past the last declared week is a genuine open question and is explicitly **not** decided here — it is sent to research (`docs/research/prompts/plan-horizon-noanchor-rollover.md`, queued in `docs/research/research-queue.md`); this ship uses `WeekContext.FromMacro`'s existing last-phase fallback as a named, degrades-gracefully-not-scientifically placeholder until that artifact lands.

**Cross-references:** `docs/plans/plan-horizon/rolling-horizon-plan.md` (full design, PR sketch, sequencing with the SPLIT UI cycle); design brief (synthesis + three rival designs + three adversarial judge panels, referenced by the plan doc); DEC-087/DEC-088 (retry-pattern precedent); DEC-074 (eval-cache binding, confirmed untriggered); `docs/planning/vision-and-principles.md` (the static-plan anti-pattern this design avoids).

---

## DEC-091: Slice 3 (Coach) log-receipt durability — a structured `LoggedRunSummary` on the confirm-ack turn; one deliberate wire delta beyond the SPLIT cycle's four (SPLIT UI cycle)

**Date:** 2026-07-14
**Category:** Conversation wire contract / frontend rendering / cycle-scope amendment
**Status:** Accepted — locks the receipt-persistence approach for `docs/specs/slice-3-coach/spec.md` (spec written + adversarially verified this session). Amends the SPLIT cycle plan's "Slice 3 is frontend-only" framing (DEC-089 / cycle-plan § Goal & Done-State).
**Drives:** how a confirmed conversational log is durably represented in the `/coach` transcript. One additive-nullable field threaded through the interactive coach-turn chain (`CoachMessagePosted` → `InteractiveTurnView`/`InteractiveConversationProjection` → `InteractiveTurnDto` → `conversation.model.ts`), populated only at the confirm seam (`ConfirmConversationalLogService`), rendered by a new `LoggedRunReceipt` on the coach text turn.
**Builds on:** DEC-085 (Slice 4B conversation core — the confirm-then-commit path and the coach-ack turn this augments); DEC-089 (the SPLIT cycle's "four narrow wire deltas" scope this amends to five); the Slice 2 `PlanGenerated` additive-nullable precedent (existing events hydrate `null` on replay, no upcaster — cycle-plan § Captured 2026-07-14).

**Problem.** The design (HANDOFF §5.2 / sheet 3b·06) ends the log-draft card in a **persistent structured receipt** — `LOGGED — 9.2 KM · 41:00 · JUL 8` + `LOG BOOK →` — that "stays in the transcript" and survives reload, and the SPLIT cycle labelled Slice 3 "frontend-only — every payload the design needs is already typed on the wire." **Recon disproved that premise.** On `POST /conversation/logs/confirm`, `ConfirmConversationalLogService` commits the workout log and persists only a **free-text coach acknowledgment turn** (`PostCoachConversationTurn` → `CoachMessagePosted`, an interactive `coach` turn carrying `content`/`isErrored` only; e.g. `ConversationAckScripts.AckUnavailable` = "Logged your run. Your plan's updated — take a look."). `ConfirmConversationalLogResponseDto` carries only `{ workoutLogId, adaptation }`. The conversation timeline has **no structured distance/time/date anywhere**; those values live only in the now-dismissed `CoachCard.draft` (in-session) and, uncorrelated, in the paginated workout-log history (`/history`). A structured receipt therefore cannot be reconstructed from the timeline on reload as the design requires — the "frontend-only" premise was false for this one sub-requirement.

**Decision:** The confirm-ack coach turn carries an **additive-nullable structured `LoggedRunSummary { WorkoutLogId, DistanceKm, DurationSeconds, OccurredOn, CompletionStatus }`**, built at the confirm site (which already holds the committed draft + `WorkoutLogId`) and populated only there — `null` on every streamed reply and every historical turn. The frontend renders the coach turn as coach text **plus** a durable receipt chip (`LoggedRunReceipt`). This keeps the conversation timeline the **single server-composed source of truth** (the controller already orders it `CreatedAt → EventVersion → TurnId`); the receipt is durable by construction (a real event-sourced turn survives reload trivially); it is faithful to the mock; and it is low-risk — an additive-nullable field on an existing Marten event (existing turns hydrate `loggedRun: null`, no upcaster), touching no LLM prompt, so **no eval re-record**. It is the **only** backend change in Slice 3; the spec regroups the plan's 2-PR sketch into 4 PRs (A backend + codegen; B/C/D frontend).

**Rejected alternatives:**
- **A — in-session receipt only; persistence = the free-text ack turn.** Smallest and strictly frontend-only, but deliberately discards durable structured domain data and degrades the designed structured receipt to prose on reload — dishonest for an app whose thesis is "the log book is the truth" (`vision-and-principles.md`).
- **B — client-side merge of `/history` receipts into the transcript.** Durable and frontend-only, but breaks the server-composed-single-timeline architecture by forcing a second client merge against a *separately paginated* infinite-scroll query, and surfaces *every* logged run (no conversational-origin discriminator exists) as a workaround for the missing field. More code, less clean, reconciles two pagination models in one scroller.
- **C-variant — a dedicated `logReceipt` turn kind** (rather than a field on the existing coach turn). Conceptually clean but a larger model change (new `ConversationTimelineTurnKind` member, new union member, new projection turn, new render-dispatch branch) for no durability gain over the additive field, and it would double the "logged" signal (ack turn + receipt turn). The additive field reuses the existing coach-turn seam with minimal surface.

**Consequences:** Slice 3 gains one deliberate backend/wire delta beyond the SPLIT cycle's fixed four (DEC-089) — Slice 3 is no longer strictly frontend-only. Eval-cache blast radius is zero (the ack content is scripted/LLM-composed and unchanged; the structured summary is deterministic from the confirmed draft). No upcaster: `CoachMessagePosted` events predating this change hydrate `LoggedRun = null` and render as plain coach text (no receipt) — acceptable, verified-empirically pattern. DEC-077 idempotency is untouched (confirm still reuses `clientMessageId`; the ack turn is still idempotent on the derived coach-turn id). Backend note surfaced during spec verification: `WorkoutDraftUnitConverter.DurationToSeconds` returns `double` and `StructuredLogDraft.OccurredOn` is already a `DateOnly`, so `DurationSeconds` is `double` on the summary and `OccurredOn` is passed through without a parse.

**Cross-references:** `docs/specs/slice-3-coach/spec.md` § 1 (full options analysis) + PR-A (exact backend threading); `docs/plans/split-ui-cycle/cycle-plan.md` § Captured During Cycle (2026-07-14 DEC-091 row); DEC-085 (confirm-then-commit + the coach-ack turn); DEC-089 (the four-wire-delta scope this amends); `docs/plans/split-ui-cycle/slice-3-coach.md` D6.

---

## DEC-092: Two-family agent routing — Codex fleets and clone builds driven from the Claude session, per-stage Codex review, the maintainer's code-gauntlet run as the cross-family pass
...
**Problem.** Every agent in this repo ran on one model family. Measured in a sibling project over one week, subagent fleets on that family were two thirds of the spend, driven by cache writes and reads from thousands of fresh subagent contexts, so the weekly window ran out before the work did. A second family reviewing the first also catches what the first normalizes: in that project the Codex lenses missed test-coverage gaps and doc drift the Claude lens found, and the Claude lens missed runtime facts the Codex lenses measured.

**Decision.** Route by what the task needs, in this order, with cost deciding only when nothing above it does: the conversation (adjudication, gates, merges, rulings) stays on the Claude session; read-only work over many items (recon over the codebase for a slice, refute lenses, extraction) goes to Codex luna at `xhigh` through the fleet driver with an API-enforced JSON schema, at any batch size, with no question to the user; a build stage is one luna `xhigh` companion task with write access in a local clone outside the repo (sol at `max` for refactors of about twenty files or more), and the session stages the brief's explicit file list and commits because Codex cannot write `.git`; per-stage review is two luna lenses (mutation ledger, spec conformance) in detached worktrees, then a luna fix round, until zero blocker and zero major; the cross-family pass on every PR is the maintainer's own code-gauntlet run in another session, never run from the build session; spec or plan red-team, and two reviewers who disagree, get one Claude opus lens plus one Codex sol `max` lens, and sol's verdict is never accepted without the Claude side; first drafts (PR bodies, decision-log entries, cycle-plan sections) and big recon reads come from a luna task with a bounded output that the session edits. Author and final refuter are never the same family. Never Fable for a subagent. A budget line runs at every catchup and before any fan-out. Codex runs only from an attended session.

Repo mechanics: `git clone --local` into a jobs directory outside the repo, the frontend `node_modules` symlinked, NuGet restored and lefthook installed by the session, the gitignored spec copied in; reviewers work in detached worktrees of the main repo after the clone's branch is fetched and its SHA compared, with the spec, the stage report, a real `node_modules`, and a restore copied in, both lenses with write access; evidence (briefs, recon and red-team JSONs, lens JSONs, adjudications, fix lists, stage reports, the outputs of orchestrator-run checks) is committed under `docs/plans/<cycle>/<slice>-evidence/`, written in the clone and committed before the build so review worktrees carry it, scrubbed of home paths before the push, while the driver's logs and ledgers stay under the jobs directory; shipping is a fetch into the main repo, a push, and `gh pr create`. Codex agents read `AGENTS.md` (sandbox facts, blocked git, the gates they can run, the house rules that bind output, the reporting contract); Claude sessions read `CLAUDE.md` and the auto-loaded rule. Templates and schemas (review context and three lenses, build brief, fix brief, recon, red-team, draft) are project files under `.claude/codex/`, rendered by one `str.format` convention.

**Measured (2026-09-04, codex-cli 0.153.0, luna):** the fleet driver with `--output-schema` returned schema-exact JSON from the main tree (one read-only item, 22.5k tokens, 14 s); a companion task in the clone wrote its file in 30 s and created no `.codex/` mirror; a write-sandbox smoke in the clone measured: `$HOME` and `~/.nuget/packages` refuse writes, `/tmp` and the workspace accept them, a write through a symlink to outside the workspace is refused (so `node_modules` must be a real directory), DNS fails, the Colima socket is unreachable, `git add` fails on the index lock, every `rm -f` form is rejected by the command policy, each command runs in a fresh shell; a plain `dotnet build` hangs after compiling until killed while the in-process flag bundle (`-nr:false -p:UseSharedCompilation=false --disable-build-servers -m:1` plus `DOTNET_CLI_HOME`, telemetry opt-out, `MSBUILDNODEREUSE=0`) passes in 6 s (a third smoke tried `--disable-build-servers` alone and the two in-process flags alone; both still hang, so the bundle stands); `dotnet test --no-build` fails on named-pipe creation even for a container-free class; `npm run build`, one vitest file, eslint and prettier pass.

**Rejected alternatives:**
- **Copy the fleet driver and budget script into the repo** (the sibling project did). The user-level skill is canonical, the copies have already drifted, and a public repo is the wrong home for tooling tied to one machine.
- **Symlink `CLAUDE.md` as `AGENTS.md`.** The audiences differ: Codex needs the sandbox and git facts and the output contract, Claude needs the process rules; one file for both would ride the cache every turn.
- **Keep review evidence gitignored with the spec.** Rejected on the sibling project's experience: a decision record months later needs the evidence path to answer why a check exists, the next slice starts from the recon facts, and a lens whose round-1 JSONs lived only in the main tree issued false majors. Rendered prompts, driver logs, and ledgers stay out; the small durable pieces go in.
- **Run the cross-family PR review from the build session.** The maintainer's code-gauntlet run is that pass (one authority per signal, DEC-043); a second run would duplicate it and put author and final refuter in one session.
- **Keep every subagent on Claude and only trim the session.** Removes the smaller lever; the fleet fan-out is the larger one.
