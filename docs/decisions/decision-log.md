...
- **D3 — Classify-then-route via a structured pre-call.** A fast Pattern-B (Haiku, `Temperature 0`) classify triages each message into `{Question | WorkoutLog | Ambiguous}` and extracts a `StructuredLogDraft` for logs. Rejected: tool-use / function-calling; an explicit `/log` affordance. On classifier failure → `Kind=Error`; never guess intent.
...
- **Streaming-posture corrections** (verified against `backend/src` during spec-writing; these correct assumptions the design doc inherited from R-082/R-083):
  ...
  - **Haiku + `Temperature` are net-new production capabilities** (Haiku is eval-judge-only today; `Temperature` is set nowhere) — the classifier adds both.
